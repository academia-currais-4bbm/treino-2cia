...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-53.js', {updateViaCache:'none'}));
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-53.js', {updateViaCache:'none'});
...
openAdmin=function(){admin52OldOpenAdmin();if(adminSessionToken)setTimeout(()=>admin52ShowTab('dashboard'),80)};
const admin52OldLogin=adminLogin;
adminLogin=async function(){await admin52OldLogin();if(adminSessionToken)setTimeout(()=>admin52ShowTab('dashboard'),80)};
