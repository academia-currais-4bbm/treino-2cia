...
function showView(id){
  ...
  if(id==='home' && typeof v67604RenderActiveWorkoutCard==='function') setTimeout(()=>v67604RenderActiveWorkoutCard(),20);
...
// V67.60.6 — após sessão estar disponível, tentativas adicionais são disparadas em pageshow/online.
window.addEventListener('online',()=>setTimeout(()=>v67606RecoverMissingFeedActivities(),500));
window.addEventListener('pageshow',()=>setTimeout(()=>{if(cloudSession?.token)v67606RecoverMissingFeedActivities()},900));
...
/* ===== V67.60.6 — RECUPERAÇÃO HISTÓRICO → FEED =====
...
let v67606FeedRecoveryRunning=false;
async function v67606RecoverMissingFeedActivities(){
  if(v67606FeedRecoveryRunning||!navigator.onLine||!cloudSession?.token)return false;
  v67606FeedRecoveryRunning=true;
  try{
    ...
    v67606FeedRecoveryRunning=false;
  }
}
...
async function v67382RefreshFeed(button){
  ...
  try{
    v6738FeedLastLoad=0;
    // V67.60.6: primeiro recupera atividades locais com atividadeId que nunca
    // chegaram ao Feed; depois mantém a reconciliação de exclusões já existente.
    await v67606RecoverMissingFeedActivities();
