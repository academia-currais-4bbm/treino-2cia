...
function v6738RenderFeed(rows){
  ...
  const cards=visible.map(r=>{
    ...
    return `<article class="sports-feed-card" data-feed-id="${Number(r.id)}">
      ...
      <footer class="feed-card-actions"><button class="feed-like ${liked?'liked':''}" type="button" onclick="v6738ToggleLike(${Number(r.id)},this)" aria-pressed="${liked?'true':'false'}"><span>${liked?'💪':'🤍'}</span><b>${likes}</b><small>${likes===1?'apoio':'apoios'}</small></button><button class="feed-comments-toggle" type="button" onclick="v6740ToggleComments(${Number(r.id)},this)" aria-expanded="false"><span>💬</span><b class="feed-comment-count">${v6740CommentCountLabel(comments)}</b></button>${own?`<button class="feed-photo-add" data-feed-photo-btn="${Number(r.id)}" type="button" onclick="v6756ChooseFeedPhoto(${Number(r.id)})"><span>📷</span><small>Foto 24h</small></button>`:''}${statusBtn}</footer>
...
/* ===== V67.56.0 — FOTO TEMPORÁRIA DO TREINO (24H) ===== */
...
function v6756SetPhotoButton(feedId,state){
  ...
  if(state==='active'){btn.disabled=true;btn.classList.add('active');btn.innerHTML='<span>📷</span><small>Foto ativa 24h</small>';return}
  btn.disabled=false;btn.classList.remove('active');btn.innerHTML='<span>📷</span><small>Foto 24h</small>';
}
...
function v6756LoadVisibleFeedPhotos(){
  ...
  ids.forEach((id,i)=>setTimeout(()=>v6756LoadFeedPhoto(id,false),Math.min(i*45,450)));
}
