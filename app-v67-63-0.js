...
/* ===== V67.63.0 — CARD OFICIAL DE COMPARTILHAMENTO ===== */
...
async function v67630BuildShareCard(record){
  ...
  const bg=x.createLinearGradient(0,0,W,H);bg.addColorStop(0,'#050505');bg.addColorStop(.52,'#120607');bg.addColorStop(1,'#020202');x.fillStyle=bg;x.fillRect(0,0,W,H);
  // luz vermelha + textura discreta
  const glow=x.createRadialGradient(110,240,20,110,240,650);glow.addColorStop(0,'rgba(220,0,18,.48)');glow.addColorStop(1,'rgba(220,0,18,0)');x.fillStyle=glow;x.fillRect(0,0,W,H);
  x.globalAlpha=.12;x.strokeStyle='#ff1f2d';x.lineWidth=2;for(let i=-H;i<W;i+=80){x.beginPath();x.moveTo(i,0);x.lineTo(i+H,H);x.stroke()}x.globalAlpha=1;
  x.fillStyle='#e3131b';x.fillRect(70,75,940,8);
  const crest=await v67630LoadImage('icon-512.png');if(crest){x.save();x.shadowColor='rgba(255,0,0,.35)';x.shadowBlur=35;x.drawImage(crest,70,115,170,170);x.restore()}
  x.textAlign='right';x.fillStyle='#fff';x.font='900 46px Arial';x.fillText('TREINO 2ª CIA',1005,160);x.fillStyle='#aaa';x.font='700 27px Arial';x.fillText('4º BBM • CURRAIS NOVOS',1005,205);x.fillStyle='#e3131b';x.fillRect(725,230,280,5);
  x.textAlign='center';x.fillStyle='#fff';x.font='900 105px Arial';x.fillText('TREINO',W/2,455);x.fillStyle='#ed151f';x.font='italic 900 126px Arial';x.fillText('CONCLUÍDO',W/2,565);x.fillStyle='#d7d7d7';x.font='700 29px Arial';x.letterSpacing='8px';x.fillText('MISSÃO CUMPRIDA',W/2,620);x.letterSpacing='0px';
  const plan=v67630ShortPlan(record);x.fillStyle='#fff';x.font=`900 ${v67630FitText(x,plan,850,50,30)}px Arial`;x.fillText(plan,W/2,735);
  const dt=v67630FmtDate(record?.date);x.fillStyle='#999';x.font='700 25px Arial';x.fillText(`${dt.date}  •  ${dt.time}`,W/2,780);
  const metrics=v67630MetricData(record), gap=18, left=70, totalW=940, boxW=(totalW-gap*(metrics.length-1))/metrics.length, y=845;
  metrics.forEach((m,i)=>{const bx=left+i*(boxW+gap);v67630RoundRect(x,bx,y,boxW,190,24,'rgba(18,18,18,.92)','#3b3b3b');x.textAlign='center';x.fillStyle='#ed151f';x.font='900 36px Arial';x.fillText(m[0],bx+boxW/2,y+52);x.fillStyle='#fff';x.font=`900 ${v67630FitText(x,m[1],boxW-24,38,23)}px Arial`;x.fillText(m[1],bx+boxW/2,y+112);x.fillStyle='#aaa';x.font='700 19px Arial';x.fillText(m[2],bx+boxW/2,y+154)});
  const prs=v6750NewRecordLines(record);let yy=1090;
  if(prs.length){v67630RoundRect(x,120,yy,840,260,28,'rgba(25,16,2,.9)','#d9a514');x.textAlign='center';x.fillStyle='#f6c62e';x.font='900 35px Arial';x.fillText(`🏆 ${prs.length} ${prs.length===1?'NOVO RECORDE':'NOVOS RECORDES'}`,W/2,yy+60);x.strokeStyle='rgba(246,198,46,.45)';x.beginPath();x.moveTo(210,yy+85);x.lineTo(870,yy+85);x.stroke();x.textAlign='left';x.fillStyle='#fff';x.font='700 29px Arial';prs.slice(0,3).forEach((p,i)=>x.fillText(p.replace(/^[^\wÀ-ÿ]+/u,''),190,yy+135+i*48));yy+=310}else{yy+=40}
  const name=v6748FirstName();v67630RoundRect(x,140,yy,800,145,72,'rgba(5,5,5,.9)','#4a0b0d');x.textAlign='center';x.fillStyle='#fff';x.font='900 42px Arial';x.fillText(name,W/2,yy+60);x.fillStyle='#999';x.font='700 22px Arial';x.fillText('TREINO 2ª CIA',W/2,yy+100);
  x.fillStyle='#e3131b';x.fillRect(70,1660,940,5);x.fillStyle='#fff';x.font='900 30px Arial';x.fillText('CORPO FORTE.  MENTE PRONTA.',W/2,1735);x.fillStyle='#e3131b';x.font='900 38px Arial';x.fillText('MISSÃO CUMPRIDA.',W/2,1785);x.fillStyle='#888';x.font='700 20px Arial';x.fillText('CORPO DE BOMBEIROS MILITAR DO RIO GRANDE DO NORTE',W/2,1850);
  return c;
}
async function v67630ShareResult(){
  ...
  try{
    ...
    const file=new File([blob],`treino-2cia-${Date.now()}.png`,{type:'image/png'});
