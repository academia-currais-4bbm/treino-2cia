...
async function v55LoadRanking(){
 ...
 try{
   ...
   const explain=isAnnual
     ? '<p><b>O Ranking Anual soma os pontos obtidos nos rankings mensais do ano.</b></p><p>Em cada janela operacional validada de 24 horas, o militar pode conquistar no máximo <b>1 ponto</b>, desde que conclua pelo menos um Treino de Força válido ou Cardio válido. O Core Operacional é complementar e não gera ponto.</p>'
     : '<p><b>Validação:</b> o período começa na validação do serviço pelo QR da unidade e permanece ativo por 24 horas.</p><p><b>Treino pronto/personalizado:</b> mínimo de 3 exercícios e 6 séries.</p><p><b>Treino livre:</b> mínimo de 20 minutos e pelo menos um grupo muscular informado.</p><p><b>Cardio:</b> mínimo de 30 minutos em Esteira, Bicicleta Ergométrica ou Simulador de Remo.</p><p><b>Core Operacional:</b> continua no Histórico, Evolução e Feed, mas não gera ponto.</p><p>Treino + Cardio, dois treinos ou vários cardios dentro da mesma janela continuam valendo no máximo <b>1 ponto</b>. Atividades fora da validação ou abaixo dos critérios valem 0 ponto.</p>';
   const quickRule=isAnnual
     ? `<div class="v55-rule-main"><span>🏆</span><div><small>REGRA PRINCIPAL</small><strong>1 validação de 24h = até 1 ponto</strong><p>O total anual é a soma dos pontos mensais.</p></div></div>`
     : `<div class="v55-rule-main"><span>🏆</span><div><small>REGRA PRINCIPAL</small><strong>1 validação de 24h = até 1 ponto</strong><p>Frequência vale mais que acumular atividades no mesmo serviço.</p></div></div>
   ...
   `<p class="v55-note">${isAnnual?'O total anual é a soma dos pontos mensais do ano. Cada janela operacional validada pode gerar no máximo 1 ponto.':'O ranking considera apenas Treino de Força ou Cardio válidos durante uma validação de serviço confirmada, com teto de 1 ponto por janela de 24 horas. Core não pontua.'} Cargas individuais não são exibidas.</p>`;
...
function renderServiceValidationCards(){
  ...
  for(const ids of targets){
    ...
    if(serviceActivityMode==='operational' && serviceValidationStatus.valid){
      ...
      if(hint)hint.textContent='Treino de Força ou Cardio válido: 1 ponto. Limite: 1 ponto nesta janela operacional de 24 horas. Core não pontua.';
      continue;
    }
...
try{localStorage.setItem('t2_app_version','v67.40.0')}catch(e){}
