...
function showView(id){
  ...
  // V67.68.9 — sair da tela de execução pausa e preserva o treino automaticamente.
  // Não interfere na conclusão/cancelamento, pois nesses fluxos o estado ativo já foi removido.
  if(previous==='exercise' && id!=='exercise'){
    const saved=v67604ActivePlanState?.();
    if(saved && saved.status!=='paused'){
      try{v67604SaveActivePlanState({status:'paused',pausedAt:new Date().toISOString()})}catch(e){}
...
/* ===== V67.68.7 — RETOMADA SEGURA DE TREINO PERSONALIZADO ===== */
function v67687DiscardBrokenActivePlan(){
  try{localStorage.removeItem('t2_active_plan')}catch(e){}
...
function v67687CanResumeActivePlan(saved){
  if(!saved||!saved.name||!saved.startedAt)return false;
  try{
    const exs=v67604PlanExercises(saved.name);
    if(!Array.isArray(exs)||!exs.length)return false;
    const idx=Math.max(0,Math.min(Number(saved.index)||0,exs.length-1));
    const target=saved.currentExerciseId ?? exs[idx]?.id;
    return !!exs.find(x=>String(x.id)===String(target));
  }catch(e){return false}
}
function v67689RemovePartialActiveWorkout(saved){
  if(!saved?.startedAt)return;
  try{
    const started=new Date(saved.startedAt).getTime();
    if(!Number.isFinite(started))return;
    const cleaned=history().filter(x=>{
      const t=new Date(x?.date).getTime();
      return !(Number.isFinite(t)&&t>=started&&x?.plan===saved.name);
    });
    saveHistory(cleaned);
...
function startPlanWorkout(){
  ...
  // Guarda o treino que o militar acabou de escolher. A limpeza/retomada de um
  // estado antigo nunca pode apagar este contexto (especialmente personalizados).
  const selectedName=activePlanName;
  const selectedExercises=activePlanExercises.slice();
  ...
  if(existing){
    ...
    try{
      ...
      hasProgress=history().some(x=>{
        const t=new Date(x?.date).getTime();
        return Number.isFinite(startedMs)&&Number.isFinite(t)&&t>=startedMs&&x?.plan===existing.name;
      });
    }catch(e){}

    if(!hasProgress){
      v67687DiscardBrokenActivePlan();
    }else if(existing.name===selectedName && v67687CanResumeActivePlan(existing)){
      if(confirm('Este treino já possui séries registradas. Deseja continuar de onde parou?')){
        if(v67604ResumeActivePlan()){v67621StopMusic(true);return;}
      }
      // Se não quiser retomar o MESMO treino, começa novamente e remove apenas
      // as séries parciais daquela tentativa anterior.
      if(!confirm('Deseja descartar o progresso parcial e iniciar este treino novamente?'))return;
      v67689RemovePartialActiveWorkout(existing);
      v67687DiscardBrokenActivePlan();
    }else{
      // Outro treino ficou salvo/pausado. Ele não pode bloquear silenciosamente
      // o treino que está sendo iniciado agora.
      const label=String(existing.name||'outro treino').replace(/^Personalizado — /,'');
      if(!confirm(`Existe um treino pausado (${label}). Deseja encerrá-lo e iniciar este treino?`))return;
      v67689RemovePartialActiveWorkout(existing);
      v67687DiscardBrokenActivePlan();
    }
  }

  activePlanName=selectedName;
  activePlanExercises=selectedExercises;
  activePlanIndex=0;
  ...
  if(!first){
    workoutStartedAt=null;activePlanIndex=-1;
    alert('Não foi possível abrir o primeiro exercício deste treino.');
    return;
  }
  openExercise(first.id,true);
  // Só persiste como treino ativo depois que a tela de execução foi aberta.
  if(document.querySelector('.view.active')?.id==='exercise'){
...
function v67686TargetReps(ex=currentExercise,setNo=currentSet){
...
function v67686RenderMethodGuide(){
  ...
  if(!currentExercise?.customMethodLabel){host.style.display='none';return}const target=v67686TargetReps();let extra='';
...
function openExercise(id,inPlan=false){
  currentExercise=(activePlanIndex>=0&&activePlanExercises[activePlanIndex]&&String(activePlanExercises[activePlanIndex].id)===String(id))?activePlanExercises[activePlanIndex]:DATA.exercises.find(x=>x.id===id); if(!currentExercise)return;
  ...
  byId('exTip').textContent=currentExercise.tip;byId('exAvoid').textContent=currentExercise.avoid;v67686RenderMethodGuide();
...
function openCustomPlan(id){
 ...
 if(startBox)startBox.innerHTML=`<button class="big red plan-start" onclick="startPlanWorkout()">▶ INICIAR TREINO</button><small>${activePlanExercises.length} exercícios • treino personalizado</small>`;showView('list')
}
