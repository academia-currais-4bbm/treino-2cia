...
/* ===== V67.60.3 — RETOMADA DURÁVEL DE TREINO EM ANDAMENTO ===== */
function v67603ActivePlanState(){
  try{
    const x=JSON.parse(localStorage.getItem('t2_active_plan')||'null');
    return x&&x.name&&x.startedAt?x:null;
  }catch(e){return null}
}
function v67603SaveActivePlanState(){
  if(!activePlanName || activePlanIndex<0 || !workoutStartedAt)return;
  const exerciseId=currentExercise?.id ?? activePlanExercises[activePlanIndex]?.id ?? null;
  const state={
    name:activePlanName,
...
function v67603ResumeActivePlan(){
  const saved=v67603ActivePlanState();
  if(!saved)return false;

  activePlanName=String(saved.name||'');
  activePlanIndex=Math.max(0,Number(saved.index)||0);
  ...
  if(DATA.plans[activePlanName]){
    activePlanExercises=DATA.plans[activePlanName].map(exByName).filter(Boolean);
  }else if(activePlanName.startsWith('Personalizado — ')){
    const customName=activePlanName.replace(/^Personalizado — /,'');
    const w=getCustomWorkouts().find(x=>x.name===customName);
    activePlanExercises=w?w.exercises.map(customWorkoutExercise).filter(Boolean):[];
  }else{
    activePlanExercises=[];
  }
  if(!activePlanExercises.length)return false;
  if(activePlanIndex>=activePlanExercises.length)activePlanIndex=activePlanExercises.length-1;
...
function restoreNavigationState(force=false){
  ...
  if(!force && !NAV_RESTORE_ON_BOOT){
    // V67.60.3: um treino realmente em andamento tem prioridade sobre a Home.
    // Isso cobre o caso em que o Android encerra o PWA enquanto ele está em segundo plano.
    if(v67603ResumeActivePlan())return;
