...
function v6742WorkoutShortMeta(w){
  if(isCardioRecord(w))return `${Number(w.duration||0)} min`;
...
function updateCoreServiceChoiceUI(){
  ...
  if(h)h.textContent=coreServiceDay===null?'Selecione uma opção antes de iniciar uma rotina.':
    (coreServiceDay?'Serviço validado pelo sistema. O Core será registrado como atividade complementar e não gera ponto no Ranking de Frequência.':'Core marcado como realizado fora do serviço. A rotina será salva normalmente e não pontuará.');
}
...
function v6738RenderFeed(rows){
  ...
  box.innerHTML=rows.map(r=>{
    ...
    const rank=r.categoria==='core'?'<span class="feed-rank neutral">🛡️ Core • não pontua no ranking</span>':(r.ranking_valido===true?'<span class="feed-rank valid">🏆 Válido para o ranking</span>':'<span class="feed-rank neutral">Registro esportivo</span>');
...
async function v55LoadRanking(){
 ...
 try{
   ...
   const explain=isAnnual
     ? '<p><b>O Ranking Anual soma os pontos obtidos nos rankings mensais do ano.</b></p><p>Em cada jornada operacional validada, das <b>07h às 07h do dia seguinte</b>, o militar pode conquistar no máximo <b>1 ponto</b>, desde que conclua pelo menos um Treino de Força válido ou Cardio válido. O Core Operacional é complementar e não gera ponto.</p>'
     : '<p><b>Validação:</b> cada serviço corresponde à jornada fixa das <b>07h às 07h do dia seguinte</b>. O horário em que o QR é validado não altera o início nem o fim da jornada.</p><p>Se o treino ocorreu antes da validação, mas dentro da mesma jornada, ele também pode ser reconhecido após a confirmação do serviço.</p><p><b>Treino pronto/personalizado:</b> mínimo de 3 exercícios e 6 séries.</p><p><b>Treino livre:</b> mínimo de 20 minutos e pelo menos um grupo muscular informado.</p><p><b>Cardio:</b> mínimo de 30 minutos em Esteira, Bicicleta Ergométrica ou Simulador de Remo.</p><p><b>Core Operacional:</b> continua no Histórico, Evolução e Feed, mas não gera ponto.</p><p>Treino + Cardio, dois treinos ou vários cardios dentro da mesma janela continuam valendo no máximo <b>1 ponto</b>. Atividades fora da validação ou abaixo dos critérios valem 0 ponto.</p>';
   const quickRule=isAnnual
     ...
     : `<div class="v55-rule-main"><span>🏆</span><div><small>REGRA PRINCIPAL</small><strong>1 jornada 07h→07h = até 1 ponto</strong><p>Frequência vale mais que acumular atividades no mesmo serviço.</p></div></div>
...
async function serviceDirectValidate(){
  ...
  try{
    ...
    if(current?.valid===true){
      ...
      await serviceFinishDirectAsOperational(`Serviço operacional já validado para esta jornada. Válido até ${serviceFmtDate(expiry)}.`);
      return;
    }
    ...
    await serviceFinishDirectAsOperational(`Serviço operacional validado para a jornada atual. Válido até ${serviceFmtDate(expiry)}.`);
  }catch(e){
    console.error(e);
...
function renderServiceValidationCards(){
  ...
  for(const ids of targets){
    ...
    if(serviceActivityMode==='operational' && serviceValidationStatus.valid){
      ...
      if(text)text.innerHTML=`Presença confirmada para esta jornada. Serviço válido até <b>${serviceFmtDate(serviceValidationStatus.expiresAt)}</b>.`;
      if(hint)hint.textContent='Treino de Força ou Cardio válido: 1 ponto. Limite: 1 ponto nesta jornada operacional das 07h às 07h. Core não pontua.';
      continue;
    }
...
async function validateServiceQr(qr){
  ...
  try{
    ...
    await refreshServiceValidationStatus();
    setServiceActivityMode('operational');
    alert(`Serviço operacional validado para a jornada atual. Válido até ${serviceFmtDate(serviceValidationStatus.expiresAt||row.expira_em)}.`);
...
try{localStorage.setItem('t2_app_version','v67.43.0')}catch(e){}
