...
function openNutrition(){showView('nutrition');nutritionLoadProfile();nutritionRenderSavedPlan();nutritionRenderEvolution();nutritionTab('sim')}
...
function nutritionMealTemplates(mode,variant){
 const sets={balanced:[['Café da manhã',['2 ovos','pão integral','1 fruta']],['Almoço',['arroz','feijão','frango grelhado','salada e legumes']],['Lanche',['iogurte natural','aveia','1 fruta']],['Jantar',['batata ou arroz','carne magra','legumes']],['Ceia',['leite ou iogurte','1 fruta']],['Refeição extra',['sanduíche integral de frango','1 fruta']]],quartel:[['Café rápido',['ovos','pão ou cuscuz','banana']],['Marmita',['arroz','feijão','frango desfiado','legumes']],['Lanche de plantão',['iogurte','aveia','banana']],['Jantar rápido',['wrap de frango','queijo','salada']],['Ceia',['leite ou iogurte','fruta']],['Extra',['sanduíche integral','frango desfiado']]],economic:[['Café da manhã',['ovos','cuscuz','banana']],['Almoço',['arroz','feijão','frango','legumes']],['Lanche',['banana','aveia','leite']],['Jantar',['cuscuz','ovos','salada']],['Ceia',['leite','fruta']],['Extra',['pão','ovos']]]};
 let a=(sets[mode]||sets.balanced).map(x=>[x[0],[...x[1]]]);if(variant%2)a=a.map((x,i)=>[x[0],x[1].map(y=>y==='frango grelhado'?'carne magra':y==='batata ou arroz'?'macaxeira ou arroz':y)]);return a;
}
function nutritionRenderSavedPlan(){const box=byId('nutritionPlanContent');if(!box)return;const p=nutritionSafeJson(NUTRITION_PLAN_KEY,null);if(!p){box.innerHTML='<div class="nutrition-evolution-card"><b>Nenhum plano gerado ainda.</b><p>Abra o Simulador, informe seus dados e gere sua primeira estimativa.</p></div>';return}const templates=nutritionMealTemplates(p.mode,p.variant||0).slice(0,Math.max(3,Math.min(6,p.meals||4))),per=Math.round(p.calories/Math.max(1,templates.length));const meals=templates.map((m,i)=>`<div class="nutrition-meal"><div class="nutrition-meal-head"><b>${m[0]}</b><span>≈ ${per} kcal</span></div><ul>${m[1].map(x=>`<li>${x}</li>`).join('')}</ul></div>`).join('');box.innerHTML=`<div class="nutrition-summary"><div class="nutrition-stat"><small>Meta diária</small><b>${p.calories} kcal</b></div><div class="nutrition-stat"><small>Gasto estimado</small><b>${p.tdee} kcal</b></div><div class="nutrition-stat"><small>Proteína</small><b>${p.protein} g</b></div><div class="nutrition-stat"><small>Carboidrato</small><b>${p.carbs} g</b></div><div class="nutrition-stat"><small>Gorduras</small><b>${p.fat} g</b></div><div class="nutrition-stat"><small>Treinos/semana</small><b>${p.training}</b></div></div><h3 class="nutrition-section-title">Sugestão de refeições</h3>${meals}${p.restrictions?`<div class="nutrition-disclaimer"><b>Preferências/restrições informadas:</b><br>${nutritionEscape(p.restrictions)}<br><br>O simulador não garante adequação clínica a alergias; confirme os alimentos antes do consumo.</div>`:''}<div class="nutrition-plan-actions"><button type="button" onclick="nutritionSwapPlan()">↻ TROCAR OPÇÕES</button><button type="button" onclick="nutritionTab('sim')">✎ AJUSTAR DADOS</button></div><div class="nutrition-disclaimer">As porções exatas variam conforme marcas e preparo. Calorias e macronutrientes são estimativas para planejamento geral.</div>`}
