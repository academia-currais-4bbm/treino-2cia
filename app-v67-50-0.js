...
function updateServiceChoiceUI(){
  ...
  if(h)h.textContent=v66WorkoutServiceDay===null?'Selecione uma opção para continuar.':
    (v66WorkoutServiceDay?'Treino com presença validada na unidade.':'Treino sem validação de presença.');
}
...
function isCardioRecord(w){return !!(w&&['treadmill','bike','rower','swim'].includes(w.cardioMachine))}
...
function cardioResetSavedState(){
  ...
  if(btn){btn.disabled=false;btn.textContent=cardioSelectedMachine==='swim'?'SALVAR NATAÇÃO':'SALVAR CARDIO'}
}
...
function cardioBindInputs(){
  ['cardioMinutes','cardioDistance','cardioCalories','cardioSwimDistance'].forEach(id=>{
...
function openCardioWorkout(){
  ...
  ['cardioMinutes','cardioDistance','cardioCalories','cardioSwimDistance'].forEach(id=>{const el=byId(id);if(el)el.value=''});
...
function saveCardioWorkout(){
  ...
  let distance=null,calories=null,swimMeters=null;
  ...
  const record={
    ...
    cardioDistanceKm:machine==='treadmill'?Math.round(distance*100)/100:(machine==='swim'?Math.round(swimMeters)/1000:null),
    ...
    cardioEligible:machine!=='swim'&&minutes>=30,cardioServiceDay:machine!=='swim'&&v66WorkoutServiceDay===true,
    serviceDay:machine!=='swim'&&v66WorkoutServiceDay===true && minutes>=30,
...
function v6749PerformanceData(){
  ...
  const achievements=[
    ...
    {id:'cardio30',icon:'❤️',name:'Cardio 30+',desc:'Faça 30 minutos de cardio em uma atividade.',ok:records.some(w=>w.cardioMachine&&w.cardioMachine!=='swim'&&Number(w.duration)>=30)},
    {id:'run5k',icon:'🏃',name:'Primeiros 5 km',desc:'Percorra 5 km na esteira.',ok:records.some(w=>w.cardioMachine==='treadmill'&&Number(w.cardioDistanceKm)>=5)},
...
function v6750Qualifies(record){
  if(record?.cardioMachine==='swim'||record?.workoutType==='core'||record?.rankingCategory==='core')return false;
...
function v6750RankingLine(record){
  ...
  if(record?.serviceDay!==true)return {icon:'📍',title:'Presença não validada',text:'Atividade registrada, mas sem ponto no ranking.',kind:'neutral'};
...
function v6750NewRecordLines(record){
  ...
  if(record?.cardioMachine==='treadmill'){
    const p=Number(record.cardioPaceSecondsPerKm||0), old=prev.filter(w=>w?.cardioMachine==='treadmill').map(w=>Number(w?.cardioPaceSecondsPerKm||0)).filter(Boolean);
...
function v6750MetricCards(record){
  ...
  }else if(record?.cardioMachine==='treadmill'){
...
localStorage.setItem('t2_app_version','v67.50.0');
...
async function v6738LoadFeed(force=false){
  ...
  try{
    ...
    await v6740LoadSocialSummary(rows);
...
function confirmPresenceOutsideUnit(){
  ...
  serviceValidationPendingChoice=false;
  setServiceActivityMode('off');
  alert('Treino registrado sem validação de presença. Ele continuará no Histórico, Evolução e Feed, mas não poderá gerar ponto no Ranking de Frequência.');
}
function renderServiceValidationCards(){
  ...
  for(const ids of targets){
    ...
    if(serviceActivityMode==='off'){
      ...
      if(title)title.textContent='TREINO SEM VALIDAÇÃO';
      if(text)text.textContent='Treino liberado normalmente para histórico e evolução. Sem validação de presença, ele não pontua no Ranking de Frequência.';
      if(hint)hint.textContent='Você pode continuar normalmente; se estiver na unidade e quiser concorrer ao ponto, valide o treino.';
      return void 0;
    }
...
try{localStorage.setItem('t2_app_version','v67.50.0')}catch(e){}
