...
function cardioResetSavedState(){
  ...
  if(btn){btn.disabled=false;btn.textContent=cardioSelectedMachine==='swim'?'SALVAR NATAÇÃO':cardioSelectedMachine==='run'?'SALVAR CORRIDA':'SALVAR CARDIO'}
}
...
function updateCardioPace(){
  ...
  if(swimEl){
    const mins=Number(byId('cardioMinutes')?.value||0),meters=Number(byId('cardioSwimDistance')?.value||0);
...
function updateCardioRankingHint(){
  ...
  if(cardioSelectedMachine==='run'){hint.className='cardio-ranking-hint';hint.textContent='🏃‍♂️ Corrida é registrada no Histórico, Evolução e Feed, mas não pontua no Ranking de Frequência.';return;}
  if(minutes>=30){
...
function saveCardioWorkout(){
  ...
  if(machine==='run'){runDistance=Number(String(byId('cardioRunDistance')?.value||'').replace(',','.'));if(!Number.isFinite(runDistance)||runDistance<=0){alert('Informe a distância percorrida na corrida.');return}}
  ...
  const record={
    ...
    swimPaceSecondsPer100m:machine==='swim'?Math.round((minutes*60)/(swimMeters/100)):null,
    cardioCalories:machine==='treadmill'?Math.round(calories):null,
    cardioPaceSecondsPerKm:machine==='treadmill'?Math.round((minutes*60)/distance):(machine==='run'?Math.round((minutes*60)/runDistance):null),
    cardioEligible:!['swim','run'].includes(machine)&&minutes>=30,cardioServiceDay:!['swim','run'].includes(machine)&&v66WorkoutServiceDay===true,
    serviceDay:!['swim','run'].includes(machine)&&v66WorkoutServiceDay===true && minutes>=30,
  ...
  const msg=byId('cardioSavedMessage');
  if(msg){
    const eligible=!['swim','run'].includes(machine)&&minutes>=30&&v66WorkoutServiceDay===true;
    msg.textContent=machine==='run'
      ? '✓ Corrida salva. Esta atividade será registrada, mas não pontuará no Ranking.'
      : machine==='swim'
  ...
  const btn=byId('saveCardioWorkoutBtn'); if(btn){btn.textContent=machine==='run'?'✓ CORRIDA SALVA':machine==='swim'?'✓ NATAÇÃO SALVA':'✓ CARDIO SALVO';btn.disabled=true}
  ['cardioMinutes','cardioDistance','cardioCalories','cardioRunDistance','cardioSwimDistance'].forEach(id=>{const el=byId(id);if(el)el.value=''});
  updateCardioRankingHint();
...
localStorage.setItem('t2_app_version','v67.54.0');
...
function v6738RenderFeed(rows){
  ...
  const cards=visible.map(r=>{
    ...
    const isSwim=planLower.includes('natação');
    const isRun=planLower.includes('corrida');
    ...
    return `<article class="sports-feed-card" data-feed-id="${Number(r.id)}">
      ...
      <div class="feed-card-body"><h4>${v6738FeedEsc(r.plano||'Atividade concluída')}</h4>${status}<div class="feed-stats">${stats.map(x=>`<span>${v6738FeedEsc(x)}</span>`).join('')}</div>${loc}<div class="feed-rank-row">${rank}</div></div>
      <footer class="feed-card-actions"><button class="feed-like ${liked?'liked':''}" type="button" onclick="v6738ToggleLike(${Number(r.id)},this)" aria-pressed="${liked?'true':'false'}"><span>${liked?'💪':'🤍'}</span><b>${likes}</b><small>${likes===1?'apoio':'apoios'}</small></button><button class="feed-comments-toggle" type="button" onclick="v6740ToggleComments(${Number(r.id)},this)" aria-expanded="false"><span>💬</span><b class="feed-comment-count">${v6740CommentCountLabel(comments)}</b></button>${statusBtn}</footer>
      <section class="feed-comments-panel" hidden></section>
    </article>`;
  }).join('');
...
function v67382RenderNotifications(rows){
  ...
  panel.innerHTML=list.slice(0,30).map(n=>{const comment=n.tipo==='comentario';return `<div class="feed-notification-item ${n.lida===true?'':'unread'}"><span class="feed-notification-icon">${comment?'💬':'💪'}</span><div><b>${v6738FeedEsc(n.ator_graduacao||'BM')} ${v6738FeedEsc(n.ator_nome||'Militar')}</b><p>${comment?'comentou na sua atividade.':'apoiou sua atividade.'}</p><small>${v6738FeedEsc(v67382NotificationWhen(n.criado_em))}</small></div></div>`}).join('');
}
...
const V6743_VAPID_PUBLIC_KEY='BJHAfYVcpeSpTmU8fo6v3AwNFL13hKRH0_XCeVqZNEN8lJ1A3H0AkKfE-tLQfUiqlkCwuIVTjRBpNbMBcrXygL8';
...
async function v6743SyncExistingPush(){
  ...
  try{const sub=await v6743CurrentSubscription();if(sub)await v6743SaveSubscription(sub)}catch(e){console.warn('Sincronização PUSH:',e)}
...
try{localStorage.setItem('t2_app_version','v67.54.0')}catch(e){}

/* ===== V67.21 — RANKING OFICIAL TAF ===== */
let officialTafState={edition:null,editions:[],rows:[],scope:'all',exercise:'pushup',view:'exercise',generalCount:3};
...
/* ===== V67.54 — RANKING GERAL TAF (3 / 4 / 5 PROVAS) ===== */
...
function officialTafSetGeneralCount(n){officialTafState.generalCount=Number(n)||3;renderOfficialTafRanking()}
function officialTafSetView(v){officialTafState.view=v==='general'?'general':'exercise';renderOfficialTafRanking()}
...
function renderOfficialTafRanking(){
 ...
 if(officialTafState.view==='general'){
   const count=officialTafState.generalCount||3;const ranked=otGeneralRanks(rows,count);
   const missingAge=rows.some(r=>!Number.isFinite(Number(r?.idade??r?.age))&&!['run_points','run_pts'].some(k=>Number.isFinite(Number(r?.[k]))));
   content=otGeneralTabs()+`<div class="card ot-rule"><b>Ranking Geral — ${count} provas</b><p>${otGeneralRule(count)} A classificação é pela soma das pontuações oficiais. Empate: menor tempo na Corrida/Natação e maior número de execuções em Barra/Abdominal/Flexão.</p></div>`+
     (ranked.length?ranked.map(otGeneralCard).join(''):`<div class="card"><p>Nenhum militar possui todos os índices necessários para este ranking${missingAge?' com idade/pontuação oficial disponível':''}.</p></div>`);
 }else{
