...
let v676813StartingPlan=false;
function startPlanWorkout(){
  // V67.68.14 — trava de reentrada: alguns WebViews/PWAs Android podem
  // disparar o acionamento do botão duas vezes no mesmo toque. A segunda
  // chamada encontrava a sessão recém-criada e mostrava, incorretamente,
  // "Já existe um treino em andamento".
  if(v676813StartingPlan)return true;
  ...
  v676813StartingPlan=true;
  const releaseStartLock=()=>setTimeout(()=>{v676813StartingPlan=false},900);
  try{
    v67670ResetActivityLocation();
    v67621StopMusic(true);

    let existing=v67604ActivePlanState();
    // Descarta apenas estado inválido/fantasma; progresso real continua protegido.
    if(existing && !v67604PlanExercises(existing.name).length){
      try{localStorage.removeItem('t2_active_plan')}catch(e){}
      existing=null;
    }
    if(existing){
      // V67.68.14 — se o estado salvo pertence ao MESMO treino que está aberto,
      // ele não é um treino concorrente. Isso também cobre reentrada/duplo disparo
      // e estados recém-criados pelo próprio botão INICIAR.
      if(String(existing.name||'')===String(activePlanName||'')){
        const loaded=v67604LoadActivePlanContext(existing);
        if(loaded){
          const ex=activePlanExercises[activePlanIndex];
          if(ex){
            let pausedTotal=Math.max(0,Number(existing.pausedTotalMs)||0);
            if(existing.status==='paused'&&existing.pausedAt){
              const p=new Date(existing.pausedAt).getTime();
              if(Number.isFinite(p))pausedTotal+=Math.max(0,Date.now()-p);
            }
            const opened=openExercise(ex.id,true);
            if(opened===false) throw new Error('Não foi possível reabrir o exercício salvo.');
            currentSet=Math.max(1,Math.min(Number(existing.currentSet)||1,currentExercise?.sets||1));
            if(byId('setLabel')&&currentExercise)byId('setLabel').textContent=`Série ${currentSet} de ${currentExercise.sets}`;
            v67604SaveActivePlanState({status:'running',pausedAt:null,pausedTotalMs:pausedTotal,currentExerciseId:ex.id,currentSet});
            saveNavigationState('exercise');
            releaseStartLock();
            return true;
          }
        }
        // Estado do mesmo treino, mas impossível de reconstruir: limpa e inicia novo.
        try{localStorage.removeItem('t2_active_plan')}catch(e){}
        existing=null;
      }
    }
    if(existing){
      // V67.68.17 — um estado antigo vindo da nuvem não pode bloquear todos os treinos.
      // Primeiro tenta retomar de verdade. Se a restauração falhar, o registro é
      // considerado órfão, é removido e o treino que o militar acabou de escolher
      // inicia normalmente no mesmo toque.
      if(confirm('Já existe outro treino em andamento. Deseja retomá-lo?')){
        const resumed=v67604ResumeActivePlan();
        if(resumed){releaseStartLock();return true;}
        try{localStorage.removeItem('t2_active_plan')}catch(e){}
        existing=null;
        activePlanIndex=-1;
    ...
    v67604SaveActivePlanState({
      ...
      pausedAt:null,
      pausedTotalMs:0,
      currentExerciseId:activePlanExercises[0].id,
      currentSet:1
    });

    // Abre diretamente o primeiro exercício da sessão que acabou de ser criada.
    // A sessão existente aqui é a PRÓPRIA sessão atual e não deve passar
    // novamente pela proteção de "treino já em andamento".
    const opened=openExercise(activePlanExercises[0].id,true);
    if(opened===false) throw new Error('Não foi possível abrir o primeiro exercício do treino.');
    saveNavigationState('exercise');
    v67604SaveActivePlanState({status:'running',pausedAt:null});
    releaseStartLock();
    return true;
  }catch(err){
    ...
    releaseStartLock();
    alert('O treino foi preservado, mas houve uma falha ao abrir o primeiro exercício. Toque em RETOMAR TREINO.');
    return false;
  }
}
...
function openExercise(id,inPlan=false){
  ...
  // V67.68.17 — entra na tela de execução IMEDIATAMENTE. Assim nenhuma rotina
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-15.js', {updateViaCache:'none'}));
...
localStorage.setItem('t2_app_version','v67.68.17');
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-15.js', {updateViaCache:'none'});
