...
function v6749RenderPerformance(celebrate=false){
  ...
  const ach=d.achievements.map(a=>`<div class="v6749-ach ${a.ok?'unlocked':'locked'}"><span>${a.ok?a.icon:'🔒'}</span><div><b>${a.name}</b><small>${a.desc}</small></div>${a.ok?'<i>CONQUISTADA</i>':''}</div>`).join('');
  box.innerHTML=`<div class="v6749-section-head"><div><span>EVOLUÇÃO PESSOAL</span><h2>🏆 Recordes pessoais</h2></div><small>Atualizados automaticamente</small></div><div class="v6749-record-grid">${records}</div><details class="v6749-achievements"><summary><span><b>🏅 Conquistas</b><small>${unlocked}/${d.achievements.length} desbloqueadas</small></span><strong>VER TODAS</strong></summary><div class="v6749-ach-grid">${ach}</div></details>`;
...
localStorage.setItem('t2_app_version','v67.49.0');
...
try{localStorage.setItem('t2_app_version','v67.49.0')}catch(e){}
