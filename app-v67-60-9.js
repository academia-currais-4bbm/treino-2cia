...
// V67.60.9 — navegação PWA/Android por URLs internas distintas.
// Em alguns PWAs instalados no Android, várias entradas pushState com a mesma URL
// podem não ser tratadas pelo botão físico Voltar como navegação interna. Esta
// versão dá a cada tela uma URL/hash própria e única, mantendo o estado completo
// no history.state. Assim o Android/Chrome tem entradas reais e distintas para
// percorrer antes de sair do aplicativo.
...
const V67609_NAV_VERSION=67609;
let v67609NavSeq=0;
function v67609Hash(viewId,seq){
  return `#t2nav-${Math.max(0,Number(seq)||0)}-${encodeURIComponent(String(viewId||'home'))}`;
}
function v67609State(viewId,depth,seq){
  return {
    t2App:true,
    t2NavVersion:V67609_NAV_VERSION,
    depth:Math.max(0,Number(depth)||0),
    seq:Math.max(0,Number(seq)||0),
    snapshot:appHistorySnapshot(viewId)
  };
}
function v67609Url(viewId,seq){
  const u=new URL(location.href);
  u.hash=v67609Hash(viewId,seq).slice(1);
  return u.pathname+u.search+u.hash;
}
function v67609CurrentDepth(){
  const st=window.history.state;
  return st?.t2App && st?.t2NavVersion===V67609_NAV_VERSION ? Math.max(0,Number(st.depth)||0) : 0;
}
function v67609InitHistory(force=false){
  try{
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(!force && st?.t2App && st?.t2NavVersion===V67609_NAV_VERSION){
      v67609NavSeq=Math.max(v67609NavSeq,Number(st.seq)||0);
      return;
    }
    v67609NavSeq=Math.max(1,Number(st?.seq)||0,Date.now()%1000000000);
    window.history.replaceState(
      v67609State(active,active==='home'?0:1,v67609NavSeq),
      '',
      v67609Url(active,v67609NavSeq)
    );
  }catch(e){console.warn('Não foi possível inicializar a navegação Android:',e)}
}
function ensureAppHistoryState(){
  try{
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(!st?.t2App || st?.t2NavVersion!==V67609_NAV_VERSION){
      v67609InitHistory(true);
    }else{
      v67609NavSeq=Math.max(v67609NavSeq,Number(st.seq)||0);
      if(st?.snapshot?.view!==active){
        window.history.replaceState(v67609State(active,v67609CurrentDepth(),st.seq||v67609NavSeq),'',v67609Url(active,st.seq||v67609NavSeq));
      }
    }
  }catch(e){}
}
...
function showView(id){
  ...
  if(!navRestoring && !browserNavHandling && previous && previous!==id){
    try{
      ...
      const st=window.history.state;
      window.history.replaceState(
        v67609State(previous,v67609CurrentDepth(),st?.seq||v67609NavSeq),
        '',
        v67609Url(previous,st?.seq||v67609NavSeq)
      );
    }catch(e){}
  }
  ...
  if(!navRestoring && !browserNavHandling && previous!==id){
    try{
      v67609NavSeq=Math.max(v67609NavSeq+1,Date.now()%1000000000);
      const nextDepth=v67609CurrentDepth()+1;
      window.history.pushState(v67609State(id,nextDepth,v67609NavSeq),'',v67609Url(id,v67609NavSeq));
    }catch(e){console.warn('Falha ao criar etapa de navegação:',e)}
  }
}
...
function appBack(fallbackView='home', fallbackAction=null){
  try{
    ...
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(st?.t2App && st?.t2NavVersion===V67609_NAV_VERSION && Number(st.depth)>0){
      window.history.replaceState(v67609State(active,Number(st.depth),st.seq||v67609NavSeq),'',v67609Url(active,st.seq||v67609NavSeq));
      window.history.back();
      return;
    }
  }catch(e){}
  ...
  if(fallbackView && fallbackView!==(document.querySelector('.view.active')?.id||'')) showView(fallbackView);
}
...
window.addEventListener('popstate',(event)=>{
  ...
  try{
    ...
    if(st?.t2App && st?.t2NavVersion===V67609_NAV_VERSION && st.snapshot){
      v67609NavSeq=Math.max(v67609NavSeq,Number(st.seq)||0);
      sessionStorage.setItem(NAV_STATE_KEY,JSON.stringify({...st.snapshot,matricula:cloudSession.matricula}));
      restoreNavigationState(true);
      return;
    }

    // Se uma entrada antiga/externa for alcançada enquanto uma tela interna está
    // visível, mantém o usuário no app e retorna à Home. Na Home, o Android pode sair.
    const active=document.querySelector('.view.active')?.id||'home';
    if(active!=='home'){
      document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',()=>v67609InitHistory(),{once:true});
}else{
  v67609InitHistory();
}
...
// V67.60.8 — após sessão estar disponível, tentativas adicionais são disparadas em pageshow/online.
window.addEventListener('online',()=>setTimeout(()=>v67608RecoverMissingFeedActivities(),500));
window.addEventListener('pageshow',()=>setTimeout(()=>{if(cloudSession?.token)v67608RecoverMissingFeedActivities()},900));
...
function restoreNavigationState(force=false){
  ...
  if(!force && !NAV_RESTORE_ON_BOOT){
    ...
    try{
      ...
      window.history.replaceState(v67609State('home',0,v67609NavSeq),'',v67609Url('home',v67609NavSeq));
    }catch(e){}
  ...
  try{
    ...
    try{
      ...
      if(activeNow!=='home' && (!currentState?.t2App || currentState?.snapshot?.view!==activeNow)){
        ...
        window.history.pushState(v67609State(activeNow,Math.max(1,v67609CurrentDepth()+1),++v67609NavSeq),'',v67609Url(activeNow,v67609NavSeq));
...
/* ===== V67.60.8 — RECUPERAÇÃO HISTÓRICO → FEED =====
...
let v67608FeedRecoveryRunning=false;
async function v67608RecoverMissingFeedActivities(){
  if(v67608FeedRecoveryRunning||!navigator.onLine||!cloudSession?.token)return false;
  v67608FeedRecoveryRunning=true;
  try{
    ...
    v67608FeedRecoveryRunning=false;
  }
}
...
async function v67382RefreshFeed(button){
  ...
  try{
    v6738FeedLastLoad=0;
    // V67.60.8: primeiro recupera atividades locais com atividadeId que nunca
    // chegaram ao Feed; depois mantém a reconciliação de exclusões já existente.
    await v67608RecoverMissingFeedActivities();
