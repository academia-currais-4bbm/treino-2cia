...
/* V67.68.35 — intenção explícita de navegação do treino personalizado.
   O checkpoint preserva progresso; esta flag decide se ele pode abrir a tela sozinho. */
const V676835_RESTORE_SUPPRESS='t2_custom_restore_suppress_v676835';
function v676835SuppressAutoRestore(on=true){try{sessionStorage.setItem(V676835_RESTORE_SUPPRESS,on?'1':'0')}catch(e){}}
function v676835AutoRestoreSuppressed(){try{return sessionStorage.getItem(V676835_RESTORE_SUPPRESS)==='1'}catch(e){return false}}
function v676835AllowAutoRestore(){v676835SuppressAutoRestore(false)}
function v676835GoHome(){
  v676835SuppressAutoRestore(true);
  try{saveNavigationState('home')}catch(e){}
  showView('home');
}
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden'){
    // V67.68.35 — grava o ponto EXATO antes de o Android/PWA suspender a página.
    try{
      const activeView=document.querySelector('.view.active')?.id||'';
      if(activeView==='exercise' && !v676835AutoRestoreSuppressed() && String(activePlanName||'').startsWith('Personalizado — ') && activePlanIndex>=0 && workoutStartedAt){
        v67604SaveActivePlanState({status:'paused',pausedAt:new Date().toISOString(),pauseReason:'background'});
        v676830WriteCheckpoint({index:activePlanIndex,currentExerciseId:currentExercise?.id??activePlanExercises[activePlanIndex]?.id??null,currentSet:Math.max(1,Number(currentSet)||1),status:'paused'});
        saveNavigationState('exercise');
      }
    }catch(e){console.warn('Falha ao salvar checkpoint ao ocultar app:',e)}
    v67623CancelResumeTimers();
    v67621StopMusic(false);
  }else{
    // Se a tela foi recriada/alterada durante a suspensão, o checkpoint local vence a Home.
    try{
      const cp=v676830ReadCheckpoint();
      if(cp&&v676830IsCustomName(cp.name)&&!v676835AutoRestoreSuppressed()){
        const activeView=document.querySelector('.view.active')?.id||'';
        if(activeView==='home'||!currentExercise||String(currentExercise.id)!==String(cp.currentExerciseId)||Number(currentSet)!==Number(cp.currentSet)){
          v676830RestoreCheckpoint(cp,false);
        }
      }
    }catch(e){console.warn('Falha ao restaurar personalizado ao retornar:',e)}
    // V67.68.38 — ao voltar do segundo plano, retoma o relógio EFETIVO do treino.
    // Antes o app gravava status=paused ao ocultar, mas não voltava para running;
    // por isso um treino completo podia ser salvo como 1 min após a primeira saída do app.
    try{
      const saved=v67604ActivePlanState();
      if(saved && saved.status==='paused' && !v676835AutoRestoreSuppressed()){
        let pausedTotal=Math.max(0,Number(saved.pausedTotalMs)||0);
        if(saved.pausedAt && saved.pauseReason!=='background'){
          const p=new Date(saved.pausedAt).getTime();
          if(Number.isFinite(p))pausedTotal+=Math.max(0,Date.now()-p);
        }
        v67604SaveActivePlanState({status:'running',pausedAt:null,pausedTotalMs:pausedTotal,pauseReason:null});
        if(v676830IsCustomName(saved.name))v676830WriteCheckpoint({status:'running'});
      }
    }catch(e){console.warn('Falha ao retomar relógio efetivo do treino:',e)}
    v67623ResumeMusicAfterReturn();
  }
});
window.addEventListener('pagehide',()=>{
  try{
    if(String(activePlanName||'').startsWith('Personalizado — ') && activePlanIndex>=0 && workoutStartedAt){
      v67604SaveActivePlanState({status:'paused',pausedAt:new Date().toISOString(),pauseReason:'background'});
      v676830WriteCheckpoint({index:activePlanIndex,currentExerciseId:currentExercise?.id??activePlanExercises[activePlanIndex]?.id??null,currentSet:Math.max(1,Number(currentSet)||1),status:'paused'});
      saveNavigationState('exercise');
    }
...
function v67610InternalBack(fallbackView='home',fallbackAction=null){
  ...
  try{
    // V67.68.55 — o CloseWatcher do Android chega aqui antes do popstate.
...
function showView(id){
  ...
  if(id==='home' && v676832BootRestoreLock && !v676832BootRestoreDone){
    setTimeout(()=>{try{v676832EnforceCustomRestore()}catch(e){}},0);
    return;
  }
...
window.addEventListener('popstate',()=>{
  ...
  const active=document.querySelector('.view.active')?.id||'home';
  // V67.68.54 — dentro da Administração, o Voltar nativo primeiro retorna ao Dashboard.
  if(active==='admin' && typeof admin54GetCurrentTab==='function' && admin54GetCurrentTab()!=='dashboard'){
    browserNavHandling=true;
    try{admin54BackToDashboard()}finally{setTimeout(()=>{browserNavHandling=false},0)}
    return;
  }
  if(active!=='home'){
    browserNavHandling=true;
...
function freeExerciseRow(name,custom=false,customKey=null){
  const key=custom?(customKey||'__outro_1__'):freeExerciseKey(name), st=freeWorkoutExerciseState[key]||{};
...
function renderFreeExercises(){
  ...
  const customKeys=Object.keys(freeWorkoutExerciseState).filter(k=>/^__outro_\d+__$/.test(k)).sort((a,b)=>(parseInt(a.match(/\d+/)?.[0]||0)-parseInt(b.match(/\d+/)?.[0]||0)));
  if(!customKeys.length)customKeys.push('__outro_1__');
  const customRows=customKeys.map(k=>freeExerciseRow('Outro exercício',true,k)).join('');
  list.innerHTML=(exercises.length?exercises.map(ex=>freeExerciseRow(ex.name)).join(''):'<p class="free-note">Nenhum exercício cadastrado para este grupo. Use “Outro exercício”.</p>')+customRows+`<button type="button" class="free-add-exercise-btn" onclick="addFreeCustomExercise()">＋ ADICIONAR OUTRO EXERCÍCIO</button>`;
}
function addFreeCustomExercise(){
  const nums=Object.keys(freeWorkoutExerciseState).map(k=>{const m=k.match(/^__outro_(\d+)__$/);return m?Number(m[1]):0});
  const n=Math.max(0,...nums)+1;
  freeWorkoutExerciseState[`__outro_${n}__`]={checked:true,name:'Outro exercício',custom:true,customName:'',sets:'',reps:'',weight:''};
  renderFreeExercises(); updateFreeEligibility(); saveNavigationState('freeWorkout');
  setTimeout(()=>{const rows=document.querySelectorAll('#freeExerciseList .free-exercise-row[data-custom="1"]');const row=rows[rows.length-1];row?.scrollIntoView({behavior:'smooth',block:'center'});row?.querySelector('.free-custom-name')?.focus()},60);
}
window.addFreeCustomExercise=addFreeCustomExercise;
...
/* ===== V67.68.35 — CHECKPOINT DURÁVEL DO TREINO PERSONALIZADO ===== */
const V676830_CUSTOM_CHECKPOINT='t2_custom_workout_checkpoint_v676830';
function v676830IsCustomName(name=activePlanName){return String(name||'').startsWith('Personalizado — ')}
function v676830ReadCheckpoint(){
  try{const x=JSON.parse(localStorage.getItem(V676830_CUSTOM_CHECKPOINT)||'null');return x&&x.name&&x.startedAt?x:null}catch(e){return null}
}
function v676830WriteCheckpoint(extra={}){
  if(!v676830IsCustomName()||activePlanIndex<0||!workoutStartedAt)return;
  const old=v676830ReadCheckpoint()||{};
  const x={...old,name:activePlanName,index:activePlanIndex,startedAt:workoutStartedAt,
    currentExerciseId:currentExercise?.id??activePlanExercises[activePlanIndex]?.id??old.currentExerciseId??null,
    currentSet:Math.max(1,Number(currentSet)||1),status:'paused',updatedAt:new Date().toISOString(),...extra};
  try{localStorage.setItem(V676830_CUSTOM_CHECKPOINT,JSON.stringify(x))}catch(e){}
}
function v676830ClearCheckpoint(){try{localStorage.removeItem(V676830_CUSTOM_CHECKPOINT)}catch(e){}}
// V67.68.35 — trava curta de boot: durante um reload com personalizado ativo,
// rotinas tardias de login/feed não podem vencer a restauração e mandar para Home.
let v676832BootRestoreLock = navigationLoadType()==='reload' && !!v676830ReadCheckpoint() && !v676835AutoRestoreSuppressed();
let v676832BootRestoreDone = false;
function v676832EnforceCustomRestore(){
  const cp=v676830ReadCheckpoint();
  if(v676835AutoRestoreSuppressed()){v676832BootRestoreLock=false;return false;}
  if(!cp||!v676830IsCustomName(cp.name)){v676832BootRestoreLock=false;return false;}
  const ok=v676830RestoreCheckpoint(cp,true);
  if(ok){v676832BootRestoreDone=true;v676832BootRestoreLock=false;}
  return ok;
}
function v676830RestoreCheckpoint(saved=v676830ReadCheckpoint(),markRunning=false){
  if(!saved||!v676830IsCustomName(saved.name))return false;
  if(!v67604LoadActivePlanContext(saved))return false;
  const idxById=activePlanExercises.findIndex(x=>String(x.id)===String(saved.currentExerciseId));
  if(idxById>=0)activePlanIndex=idxById;
  const ex=activePlanExercises[activePlanIndex]; if(!ex)return false;
  navRestoring=true;
  try{
    // Define a série ANTES de abrir a tela. openExercise respeita este valor quando navRestoring=true.
    currentSet=Math.max(1,Math.min(Number(saved.currentSet)||1,Number(ex?.sets)||1));
    openExercise(ex.id,true);
    currentSet=Math.max(1,Math.min(Number(saved.currentSet)||1,Number(currentExercise?.sets)||1));
    if(typeof v676822ApplyExecutionCoreUI==='function')v676822ApplyExecutionCoreUI(false);
    if(typeof v67612UpdateWorkoutVisuals==='function')v67612UpdateWorkoutVisuals();
    if(typeof v67625SyncCurrentSetUI==='function')v67625SyncCurrentSetUI();
    pauseTimer(); timer=Math.max(0,Number(currentExercise?.rest)||0); updateClock();
    const merged={...saved,index:activePlanIndex,currentExerciseId:ex.id,currentSet,status:markRunning?'running':'paused'};
    try{localStorage.setItem('t2_active_plan',JSON.stringify(merged))}catch(e){}
    v676830WriteCheckpoint({index:activePlanIndex,currentExerciseId:ex.id,currentSet,status:markRunning?'running':'paused'});
    saveNavigationState('exercise');
...
function v67604SaveActivePlanState(extra={}){
  ...
  if(v676830IsCustomName(state.name)) v676830WriteCheckpoint({index:state.index,currentExerciseId:state.currentExerciseId,currentSet:state.currentSet,status:state.status});
...
function v67604EffectiveElapsedMs(saved=v67604ActivePlanState(), now=Date.now()){
  ...
  if(saved.status==='paused'&&saved.pausedAt&&saved.pauseReason!=='background'){
    const p=new Date(saved.pausedAt).getTime();
...
function v67604PauseWorkout(goHome=false){
  if(goHome)v676835SuppressAutoRestore(true);
  const saved=v67604ActivePlanState();
  if(!saved)return false;
  pauseTimer();
  if(saved.status!=='paused')v67604SaveActivePlanState({status:'paused',pausedAt:new Date().toISOString(),pauseReason:'user'});
  if(goHome){v676835GoHome();setTimeout(v67604RenderActiveWorkoutCard,20)}
  return true;
}
function v67604ResumeActivePlan(){
  v676835AllowAutoRestore();
  const saved=(v676830ReadCheckpoint()&&v676830IsCustomName(v676830ReadCheckpoint()?.name))?v676830ReadCheckpoint():v67604ActivePlanState();
  if(!saved)return false;
  if(!v67604LoadActivePlanContext(saved))return false;
  let pausedTotal=Math.max(0,Number(saved.pausedTotalMs)||0);
  if(saved.status==='paused'&&saved.pausedAt&&saved.pauseReason!=='background'){
  ...
  if(typeof v676822ApplyExecutionCoreUI==='function' && v676830IsCustomName())v676822ApplyExecutionCoreUI(false);
  if(typeof v67625SyncCurrentSetUI==='function')v67625SyncCurrentSetUI();
  if(byId('setLabel')&&currentExercise)byId('setLabel').textContent=`Série ${currentSet} de ${currentExercise.sets}`;
  v676861SyncSkipControls();
  setTimeout(v676861SyncSkipControls,80);
  v67604SaveActivePlanState({status:'running',pausedAt:null,pausedTotalMs:pausedTotal,pauseReason:null});
  saveNavigationState('exercise');
  return true;
}
...
// V67.68.39 — corrige registros estruturados afetados pelo bug antigo de duração = 1 min.
// Só atua quando há evidência forte: treino com >=5 séries, duração salva <=1 min e
// intervalo real startedAt→date de pelo menos 5 minutos.
function v676839RepairWorkoutDurations(list){
  let changed=false;
  const out=(Array.isArray(list)?list:[]).map(w=>{
    if(!w||typeof w!=='object')return w;
    const start=new Date(w.startedAt||'').getTime(), end=new Date(w.date||'').getTime();
    const wall=Number.isFinite(start)&&Number.isFinite(end)&&end>start?Math.round((end-start)/60000):0;
    const strength=!w.cardioMachine && w.workoutType!=='free' && w.workoutType!=='core';
    if(strength && Number(w.duration||0)<=1 && Number(w.sets||0)>=5 && wall>=5){changed=true;return {...w,duration:wall,durationRepairedV676839:true,feedDurationRepairPending:true};}
    return w;
  });
...
function workoutHistory(){
  try{
    const x=JSON.parse(localStorage.getItem('t2_workouts')||'[]');
    const fixed=v676839RepairWorkoutDurations(Array.isArray(x)?x:[]);
    if(fixed.changed){
      localStorage.setItem('t2_workouts',JSON.stringify(fixed.list));
      // V67.68.40 — a correção local da duração também precisa voltar ao Feed.
      // A RPC faz UPSERT por atividade_id, atualizando duracao e o JSON da atividade.
      try{fixed.list.filter(w=>w?.feedDurationRepairPending===true).forEach(w=>v6738QueueFeedActivity(w))}catch(e){console.warn('Falha ao reenfileirar duração corrigida no Feed:',e)}
      try{if(typeof cloudScheduleSync==='function')cloudScheduleSync()}catch(e){}
    }
    return fixed.list;
...
async function finishPlanWorkout(){
  ...
  let minutes=Math.max(1,Math.round(effectiveMs/60000));
  const metrics=getActiveWorkoutMetrics();
  // V67.68.40 — proteção contra estado de pausa inconsistente no Android/PWA.
  // Se um treino estruturado realmente durou vários minutos e possui várias séries,
  // nunca salva 1 min apenas porque o estado persistido ficou incorreto.
  const wallMinutes=Math.max(0,Math.round((ended-started)/60000));
  if(minutes<=1 && metrics.sets>=5 && wallMinutes>=5)minutes=wallMinutes;
...
/* ===== V67.68.61 — RESTAURA CONTROLES DE PULO NO TREINO PERSONALIZADO ===== */
function v676861SyncSkipControls(){
  const inPlan = activePlanIndex >= 0 && Array.isArray(activePlanExercises) && activePlanExercises.length > 0;
  const skipSet = byId('skipSetBtn');
  const skipExercise = byId('skipExerciseBtn');
  [skipSet, skipExercise].forEach(btn=>{
    if(!btn)return;
    btn.hidden = !inPlan;
    btn.disabled = false;
    btn.style.display = inPlan ? 'block' : 'none';
    btn.style.visibility = inPlan ? 'visible' : 'hidden';
    btn.style.opacity = inPlan ? '1' : '';
  });
}
window.v676861SyncSkipControls=v676861SyncSkipControls;
...
function openExercise(id,inPlan=false){
  ...
  showView('exercise');
  // V67.68.35 — durante restauração, NUNCA zera a série para 1.
  // O checkpoint é a fonte de verdade e deve sobreviver ao reload completo.
  const v676832RestoringSet = navRestoring && v676830IsCustomName() ? Math.max(1,Number(currentSet)||1) : 1;
  currentGroup=currentExercise.group;currentSet=v676832RestoringSet;timer=currentExercise.rest;pauseTimer();
  // Em abertura normal inicia em 1; em restauração preserva a série do checkpoint.
  if(String(activePlanName||'').startsWith('Personalizado — ')) v676822ApplyExecutionCoreUI(!navRestoring);
  ...
  v676861SyncSkipControls();
  showView('exercise');
  // Reforça após renderizações tardias/restauração do personalizado no PWA.
  try{requestAnimationFrame(v676861SyncSkipControls)}catch(e){}
  setTimeout(v676861SyncSkipControls,40);
  setTimeout(v676861SyncSkipControls,180);
  return true;
}
function history(){
...
function skipCurrentSet(){
  ...
  if(setNo<totalSets){
    currentSet=setNo+1;
    if(v676830IsCustomName())v676830WriteCheckpoint({index:activePlanIndex,currentExerciseId:currentExercise?.id,currentSet,status:'running'});
    const label=byId('setLabel'); if(label)label.textContent=`Série ${currentSet} de ${totalSets}`;
...
function skipCurrentExercise(){
  ...
  pauseTimer();
  nextPlanExercise();
  // V67.68.36 — mesmo comportamento dos treinos prontos: ao pular um
  // exercício e existir próximo exercício, o descanso começa automaticamente.
  if(activePlanIndex>=0 && currentExercise) v676827StartRestLikeReadyWorkout();
  return true;
}
...
function v6750ClosePostWorkout(goHome=false){
  document.getElementById('v6750PostWorkout')?.classList.remove('show');
  if(goHome){
    v676835SuppressAutoRestore(true);
    try{v676830ClearCheckpoint()}catch(e){}
    try{localStorage.removeItem('t2_active_plan')}catch(e){}
...
function v6750OpenFeed(){
  // V67.68.36 — saída pós-treino é definitiva: fecha o modal, invalida qualquer
  // restauração do treino concluído e abre explicitamente a Home antes do Feed.
  v6750ClosePostWorkout(false);
  v676835SuppressAutoRestore(true);
  try{v676830ClearCheckpoint()}catch(e){}
  try{localStorage.removeItem('t2_active_plan')}catch(e){}
  activePlanIndex=-1; workoutStartedAt=null;
  showView('home');
  try{saveNavigationState('home')}catch(e){}
  try{if(typeof v6738LoadFeed==='function')v6738LoadFeed(true)}catch(e){}
  setTimeout(()=>document.querySelector('.sports-feed-shell')?.scrollIntoView({behavior:'smooth',block:'start'}),260);
}
...
/* ===== V67.68.47 — METAS PESSOAIS + CONQUISTAS NA EVOLUÇÃO ===== */
const V676847_GOALS_KEY='t2_personal_goals_v676847';
function v676847Goals(){
  let g={monthlyWorkouts:12,monthlyCardio:20};
  try{const x=JSON.parse(localStorage.getItem(V676847_GOALS_KEY)||'null');if(x&&typeof x==='object')g={...g,...x}}catch(e){}
  g.monthlyWorkouts=Math.max(1,Math.min(60,Math.round(Number(g.monthlyWorkouts)||12)));
  g.monthlyCardio=Math.max(1,Math.min(500,Number(g.monthlyCardio)||20));
  return g;
}
function v676847EditGoal(type){
  const g=v676847Goals(), isWorkout=type==='workouts';
  const current=isWorkout?g.monthlyWorkouts:g.monthlyCardio;
  const raw=prompt(isWorkout?'Meta de treinos para este mês:':'Meta de cardio no mês (km):',String(current).replace('.',','));
  if(raw===null)return;
  const n=Number(String(raw).replace(',','.'));
  if(!Number.isFinite(n)||n<=0){alert('Digite uma meta válida maior que zero.');return}
  if(isWorkout)g.monthlyWorkouts=Math.max(1,Math.min(60,Math.round(n)));else g.monthlyCardio=Math.max(1,Math.min(500,Math.round(n*10)/10));
  localStorage.setItem(V676847_GOALS_KEY,JSON.stringify(g));renderProgress();
}
...
/* ===== V67.68.44 — CENTRAL DE DESEMPENHO ===== */
function v676844MonthKey(d){return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}
function v676844PerformanceHub(records){
  const box=byId('v676844PerformanceHub'); if(!box)return;
  const now=new Date(), currentKey=v676844MonthKey(now);
  const prev=new Date(now.getFullYear(),now.getMonth()-1,1), prevKey=v676844MonthKey(prev);
  const month=records.filter(w=>{const d=new Date(w.date);return !Number.isNaN(d.getTime())&&v676844MonthKey(d)===currentKey});
  const previous=records.filter(w=>{const d=new Date(w.date);return !Number.isNaN(d.getTime())&&v676844MonthKey(d)===prevKey});
  const minutes=Math.round(month.reduce((a,w)=>a+Number(w.duration||0),0));
  const volume=Math.round(month.reduce((a,w)=>a+Number(w.volume||0),0));
  const service=month.filter(w=>w.serviceDay===true).length;
  const cardioKm=month.reduce((a,w)=>a+Number(w.cardioDistanceKm||0),0);
  const delta=previous.length?Math.round((month.length-previous.length)/previous.length*100):null;
  const prs=personalRecords();
  const monthPrs=prs.filter(p=>p.previous>0&&p.gain>0).filter(p=>{const d=new Date(p.date);return !Number.isNaN(d.getTime())&&v676844MonthKey(d)===currentKey});
  const perf=v6749PerformanceData();
  const unlocked=perf.achievements.filter(a=>a.ok).length;
  const totalAch=perf.achievements.length;
  const monthName=now.toLocaleDateString('pt-BR',{month:'long'});
  const comparison=delta===null?'Primeiro mês com base comparável':delta===0?'Mesmo número de treinos do mês anterior':`${delta>0?'▲':'▼'} ${Math.abs(delta)}% em treinos vs. mês anterior`;
  box.innerHTML=`
    <div class="v676844-hero">
      <div><span>CENTRAL DE DESEMPENHO</span><h2>Seu mês em números</h2><p>${monthName.charAt(0).toUpperCase()+monthName.slice(1)} • dados dos seus registros válidos</p></div>
      <b>${month.length}</b>
    </div>
    <div class="v676844-grid">
      <div><small>⏱️ TEMPO TOTAL</small><strong>${minutes} min</strong><span>${Math.floor(minutes/60)}h ${minutes%60}min</span></div>
      <div><small>🏋️ VOLUME</small><strong>${volume.toLocaleString('pt-BR')} kg</strong><span>carga registrada</span></div>
      <div><small>🛡️ EM SERVIÇO</small><strong>${service}</strong><span>treino${service===1?'':'s'} validado${service===1?'':'s'}</span></div>
      <div><small>🏃 CARDIO</small><strong>${cardioKm.toLocaleString('pt-BR',{maximumFractionDigits:1})} km</strong><span>distância acumulada</span></div>
    </div>
...
function renderProgress(){
  ...
  const trackedExercises=new Set(wh.flatMap(w=>getWorkoutSets(w)).filter(x=>Number(x.weight)>0&&!isBodyweightName(x.exercise)).map(x=>x.exercise)).size;
  byId('progressSummary').innerHTML=`<div class="stat"><strong>${trackedExercises}</strong><small>exercícios acompanhados</small></div><div class="stat"><strong>${reliableSeries}</strong><small>séries válidas</small></div><div class="stat"><strong>${days.size}</strong><small>dias treinados</small></div><div class="stat"><strong>${Math.round(totalVolume).toLocaleString('pt-BR')}</strong><small>kg de volume</small></div>`;
  ...
  const prs=personalRecords();
  const brokenPrs=prs.filter(p=>p.previous>0 && p.gain>0);
  const prBox=byId('personalRecords');
  if(prBox) prBox.innerHTML=brokenPrs.length?brokenPrs.slice(0,12).map(p=>`<div class="pr-card pr-new">
      ...
      <div class="pr-meta">Anterior: ${p.previous} kg <span>→</span> Atual: ${p.weight} kg <strong>+${p.gain.toFixed(p.gain%1?1:0)} kg (+${p.pct.toFixed(1)}%)</strong></div>
    </div>`).join(''):'<p class="muted">Quando você superar uma melhor carga anterior, o novo PR aparecerá aqui.</p>';
  ...
  byId('volumeChart').innerHTML=recent.length?recent.map(x=>{
    ...
    const planLabel=String(x.plan).split('—')[0].trim();
    return `<div class="chart-row"><span class="chart-label" title="${planLabel.replaceAll('&','&amp;').replaceAll('\"','&quot;')}">${planLabel}</span><div class="bar-track"><i style="width:${pct}%"></i></div><b>${Math.round(Number(x.volume||0)).toLocaleString('pt-BR')} kg</b></div>`;
...
function renderExerciseProgress(name){
  ...
  const daily={};
  pts.forEach(x=>{
    const d=x.date.slice(0,10), w=Number(x.weight), reps=Number(x.reps)||0;
    if(!daily[d] || w>daily[d].weight || (w===daily[d].weight && reps>daily[d].reps)) daily[d]={weight:w,reps};
  });
  const entries=Object.entries(daily).slice(-12);
  const box=byId('exerciseProgressChart');
  if(!entries.length){box.innerHTML='<p class="muted">Sem dados para este exercício.</p>';return;}
  const first=entries[0][1].weight;
  const best=Math.max(...entries.map(([,v])=>v.weight));
  const diff=best-first, pct=first>0?diff/first*100:0;
  const max=Math.max(best,1);
  const bestPerformance=entries.slice().sort((a,b)=>(b[1].weight-a[1].weight)||(b[1].reps-a[1].reps))[0][1];
  const summary=`<div class="exercise-summary">
    <div><span>Primeira carga</span><b>${first} kg</b></div>
    <div><span>Melhor carga</span><b>${best} kg</b></div>
    <div class="${diff>0?'gain-positive':'gain-neutral'}"><span>Evolução de carga</span><b>${diff>0?'+':''}${diff} kg • ${diff>0?'+':''}${pct.toFixed(1)}%</b></div>
    <div class="best-performance"><span>Melhor desempenho</span><b>${bestPerformance.weight} kg${bestPerformance.reps?` × ${bestPerformance.reps} reps`:''}</b></div>
  </div>`;
  const timeline=`<div class="progress-timeline v676846-timeline">${entries.map(([d,v],i)=>{
    const prev=i?entries[i-1][1]:null, delta=prev?v.weight-prev.weight:null;
    return `<div class="progress-point">
      ...
      <div class="progress-track"><i style="width:${Math.max(6,Math.round(v.weight/max*100))}%"></i></div>
      <div class="progress-value"><b>${v.weight} kg</b><small>${v.reps?`${v.reps} reps`:''}${delta===null?'':` • <span class="${delta>0?'up':delta<0?'down':'same'}">${delta>0?'+':''}${delta} kg</span>`}</small></div>
    </div>`;
  }).join('')}</div>`;
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-61.js', {updateViaCache:'none'}));
...
localStorage.setItem('t2_app_version','v67.68.50');
...
function restoreNavigationState(force=false){
  ...
  if(durableCustom && v676830IsCustomName(durableCustom.name)){
    try{if(v676830RestoreCheckpoint(durableCustom,false))return}catch(e){console.warn('Falha ao restaurar checkpoint personalizado:',e)}
  }
  ...
  // V67.68.35 — ao ATUALIZAR a página durante um treino estruturado/personalizado,
  // a sessão ativa é a fonte de verdade. Não deixa o reload cair na Home por um
  // snapshot de navegação ausente/antigo.
  if(!force && NAV_RESTORE_ON_BOOT){
    const active=v67604ActivePlanState();
    if(active){
      try{
        if(v67604LoadActivePlanContext(active)){
          const ex=activePlanExercises[activePlanIndex];
          if(ex){
            navRestoring=true;
            openExercise(ex.id,true);
            currentSet=Math.max(1,Math.min(Number(active.currentSet)||1,currentExercise?.sets||1));
            if(byId('setLabel')&&currentExercise)byId('setLabel').textContent=`Série ${currentSet} de ${currentExercise.sets}`;
            v67604SaveActivePlanState({status:active.status||'running'});
            window.scrollTo({top:0,behavior:'auto'});
            return;
          }
        }
      }catch(e){console.warn('Falha ao restaurar treino ativo após atualização:',e)}
      finally{navRestoring=false}
    }
  }
  ...
  // No reload normal, a cópia congelada vence qualquer gravação tardia da Home
  // feita durante o boot. Em navegação forçada/Voltar, continua usando o snapshot atual.
  if(!force && NAV_RESTORE_ON_BOOT && NAV_BOOT_RELOAD_SNAPSHOT?.matricula===cloudSession.matricula){
    state=NAV_BOOT_RELOAD_SNAPSHOT;
    NAV_BOOT_RELOAD_SNAPSHOT=null; // usa uma única vez; depois a navegação volta ao fluxo normal
  }
...
function v6738QueueFeedActivity(record){
  ...
  // V67.68.40 — se a mesma atividade já estiver pendente, substitui pela versão
  // mais nova. Isso permite corrigir duration sem manter um payload antigo (=1).
  const idx=items.findIndex(x=>(item.activityId&&x.activityId===item.activityId)||(!item.activityId&&x.sourceKey===item.sourceKey));
...
/* ===== V67.68.49 — FEED ESPORTIVO PRO: filtros, ordenação e resumo ===== */
let v6848FeedFilter='all';
let v6848FeedSort='recent';
function v6848SetFeedFilter(value,button){
  v6848FeedFilter=['all','treino','cardio','servico','meus'].includes(value)?value:'all';
  document.querySelectorAll('[data-feed-filter]').forEach(b=>b.classList.toggle('active',b.dataset.feedFilter===v6848FeedFilter));
  v6738RenderFeed(v6753FeedRows);
}
function v6848SetFeedSort(value){v6848FeedSort=value==='engagement'?'engagement':'recent';v6738RenderFeed(v6753FeedRows)}
function v6848FilterRows(rows){
  const myMat=cloudNormalize(cloudSession?.matricula||'');
  let list=Array.isArray(rows)?rows.slice():[];
  if(v6848FeedFilter==='treino')list=list.filter(r=>r.categoria==='treino');
  else if(v6848FeedFilter==='cardio')list=list.filter(r=>r.categoria==='cardio');
  else if(v6848FeedFilter==='servico')list=list.filter(r=>r.ranking_valido===true||r.service_day===true||r.serviceDay===true);
  else if(v6848FeedFilter==='meus')list=list.filter(r=>myMat&&cloudNormalize(r?.matricula||'')===myMat);
  if(v6848FeedSort==='engagement')list.sort((a,b)=>{
    const sa=(Number(a.curtidas)||0)+(Number(v6740SocialFor(a.id).comentarios_count)||0);
    const sb=(Number(b.curtidas)||0)+(Number(v6740SocialFor(b.id).comentarios_count)||0);
    return sb-sa || new Date(b.data_atividade)-new Date(a.data_atividade);
  });
  else list.sort((a,b)=>new Date(b.data_atividade)-new Date(a.data_atividade));
  return list;
}
function v6848RenderFeedSummary(rows){
  const el=byId('v6848FeedSummary');if(!el)return;const list=Array.isArray(rows)?rows:[];
  if(!list.length){el.hidden=true;return}
  const supports=list.reduce((n,r)=>n+(Number(r.curtidas)||0),0);
  const comments=list.reduce((n,r)=>n+(Number(v6740SocialFor(r.id).comentarios_count)||0),0);
  const people=new Set(list.map(r=>cloudNormalize(r?.matricula||r?.nome||'')).filter(Boolean)).size;
  el.hidden=false;el.innerHTML=`<div><b>${list.length}</b><span>atividades</span></div><div><b>${people}</b><span>militares</span></div><div><b>${supports}</b><span>apoios</span></div><div><b>${comments}</b><span>comentários</span></div>`;
}
...
/* ===== V67.68.50 — DESTAQUES SOCIAIS: PRs + CONQUISTAS NO FEED ===== */
function v6850ActivityJson(r){
  const a=r?.atividade??r?.atividade_json??null;
  if(a&&typeof a==='object')return a;
  if(typeof a==='string'){try{return JSON.parse(a)}catch(e){}}
  return {};
}
function v6850FeedHighlights(row,allRows){
  const mat=cloudNormalize(row?.matricula||''); if(!mat)return [];
  const when=new Date(row?.data_atividade||'').getTime();
  const same=(Array.isArray(allRows)?allRows:[]).filter(x=>cloudNormalize(x?.matricula||'')===mat&&Number(x?.id)!==Number(row?.id));
  const prior=same.filter(x=>new Date(x?.data_atividade||'').getTime()<when);
  const out=[]; const count=prior.length+1;
  const milestones={1:['🎖️','Primeira atividade'],10:['🏅','10 atividades'],25:['🥉','25 atividades'],50:['🥈','50 atividades'],100:['🥇','100 atividades']};
  if(milestones[count])out.push({kind:'achievement',icon:milestones[count][0],title:'CONQUISTA',text:milestones[count][1]});
  const a=v6850ActivityJson(row);
  const volume=Number(a?.volume||row?.volume||0);
  const priorVolumes=prior.map(x=>Number(v6850ActivityJson(x)?.volume||x?.volume||0)).filter(x=>x>0);
  if(volume>0&&priorVolumes.length&&volume>Math.max(...priorVolumes))out.push({kind:'pr',icon:'⚡',title:'NOVO PR',text:`Volume: ${Math.round(volume).toLocaleString('pt-BR')} kg`});
  let maxKg=0;(Array.isArray(a?.byExercise)?a.byExercise:[]).forEach(x=>maxKg=Math.max(maxKg,Number(x?.maxWeight||0)));
  const priorKg=prior.map(x=>{let m=0;(Array.isArray(v6850ActivityJson(x)?.byExercise)?v6850ActivityJson(x).byExercise:[]).forEach(e=>m=Math.max(m,Number(e?.maxWeight||0)));return m}).filter(Boolean);
  if(maxKg>0&&priorKg.length&&maxKg>Math.max(...priorKg))out.push({kind:'pr',icon:'🏋️',title:'NOVO PR',text:`Carga: ${maxKg.toLocaleString('pt-BR')} kg`});
  const dist=Number(row?.distancia_km||a?.cardioDistanceKm||0);
  const priorDist=prior.filter(x=>x?.categoria==='cardio').map(x=>Number(x?.distancia_km||v6850ActivityJson(x)?.cardioDistanceKm||0)).filter(x=>x>0);
  if(row?.categoria==='cardio'&&dist>0&&priorDist.length&&dist>Math.max(...priorDist))out.push({kind:'pr',icon:'🏃',title:'NOVO PR',text:`Distância: ${dist.toLocaleString('pt-BR',{maximumFractionDigits:2})} km`});
  return out.slice(0,2);
}
function v6850HighlightHtml(row,allRows){
  const h=v6850FeedHighlights(row,allRows); if(!h.length)return '';
  return `<div class="v6850-feed-highlights">${h.map(x=>`<div class="v6850-highlight ${x.kind}"><span>${x.icon}</span><div><b>${x.title}</b><small>${v6738FeedEsc(x.text)}</small></div></div>`).join('')}</div>`;
}
function v6738RenderFeed(rows){
  ...
  const periodRows=v6753FeedVisibleRows(v6753FeedRows);
  const visible=v6848FilterRows(periodRows);
  v6848RenderFeedSummary(visible);
  if(!visible.length){box.innerHTML='<div class="sports-feed-state">Nenhuma atividade encontrada neste filtro nos últimos '+v6753FeedDays+' dias.</div>';return}
  const cards=visible.map(r=>{
    ...
    return `<article class="sports-feed-card" data-feed-id="${Number(r.id)}">
      ...
      <div class="feed-card-body"><h4>${v6738FeedEsc(r.plano||'Atividade concluída')}</h4>${status}${v6850HighlightHtml(r,v6753FeedRows)}<div class="feed-stats">${stats.map(x=>`<span>${v6738FeedEsc(x)}</span>`).join('')}</div>${loc}<div class="feed-rank-row">${rank}</div><div class="feed-temp-photo" data-feed-photo="${Number(r.id)}" hidden></div></div>
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-61.js', {updateViaCache:'none'});
...
function cloudHideGate(afterLogin=false){
  ...
  // V67.68.35 — revalida após as rotinas assíncronas de boot/login. Isso impede
  // que uma chamada posterior à Home desfaça a tela restaurada.
  if(!v676835AutoRestoreSuppressed() && (v676832BootRestoreLock || (navigationLoadType()==='reload' && v676830ReadCheckpoint()))){
    [40,180,600,1400].forEach(ms=>setTimeout(()=>{try{v676832EnforceCustomRestore()}catch(e){}},ms));
  }
...
async function cloudInitialSync(freshLogin=false){
  ...
  try{
    ...
    finalData.workouts=v676839RepairWorkoutDurations(finalData.workouts).list;
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden'){
    ...
    const active=v67604ActivePlanState();
    if(active && active.status!=='paused'){ pauseTimer(); v67604SaveActivePlanState({status:'paused',pausedAt:new Date().toISOString(),pauseReason:'background'}); }
    else v67604SaveActivePlanState();
    saveNavigationState(document.querySelector('.view.active')?.id||'home');
  }
  if(document.visibilityState==='visible'){
    const cp=v676830ReadCheckpoint();
    if(cp&&v676830IsCustomName(cp.name)){
...
function v6718RenderHomeProfilePhoto(){
  ...
  // V67.68.51 — a foto da Home abre o próprio Perfil Esportivo.
  badge.setAttribute('role','button');
  badge.setAttribute('tabindex','0');
  badge.setAttribute('aria-label','Abrir meu Perfil Esportivo');
  badge.onclick=()=>v6851OpenOwnSportsProfile();
  badge.onkeydown=(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();v6851OpenOwnSportsProfile()}};
}
async function v6717ChooseProfilePhoto(input,returnToSports=false){
  ...
  try{
    ...
    v6718RenderHomeProfilePhoto();
    try{await v67381SyncFeedProfile()}catch(e){}
    input.value='';
    if(returnToSports){
      const own=String(cloudSession?.matricula||'').trim();
      if(own)await v6757OpenSportsProfile(own,{fromRestore:true});
    }else openV55Profile();
  }catch(e){
...
try{localStorage.setItem('t2_app_version','v67.68.51')}catch(e){}
...
// V67.68.51 — Perfil Esportivo persistente + Voltar nativo + perfil próprio pela Home.
const V6851_SPORTS_PROFILE_KEY='t2_sports_profile_v6851';
let v6851SportsProfileMatricula='';
let v6851SportsProfileHistoryArmed=false;
let v6851SportsProfileClosing=false;
let v6851SportsProfileWatcher=null;
function v6851IsOwnProfile(matricula){return String(matricula||'').trim()===String(cloudSession?.matricula||'').trim()}
function v6851SaveSportsProfile(matricula){try{sessionStorage.setItem(V6851_SPORTS_PROFILE_KEY,String(matricula||''))}catch(e){}}
function v6851ClearSportsProfile(){try{sessionStorage.removeItem(V6851_SPORTS_PROFILE_KEY)}catch(e){}}
function v6851OpenOwnSportsProfile(){
  const matricula=String(cloudSession?.matricula||'').trim();
  if(!matricula){alert('Entre com sua matrícula para visualizar seu Perfil Esportivo.');return}
  v6757OpenSportsProfile(matricula);
}
function v6851ArmSportsProfileBack(){
  try{v6851SportsProfileWatcher?.destroy?.()}catch(e){} v6851SportsProfileWatcher=null;
  if(!('CloseWatcher' in window))return;
  try{
    const w=new CloseWatcher();v6851SportsProfileWatcher=w;
    w.addEventListener('cancel',e=>{try{if(e.cancelable)e.preventDefault()}catch(_){};v6757CloseSportsProfile(true)});
    w.addEventListener('close',()=>{if(v6851SportsProfileWatcher===w)v6851SportsProfileWatcher=null;if(byId('v6757SportsProfileModal')?.classList.contains('show'))v6757CloseSportsProfile(true)});
  }catch(e){}
}
function v6851RestoreSportsProfile(){
  let matricula='';try{matricula=String(sessionStorage.getItem(V6851_SPORTS_PROFILE_KEY)||'').trim()}catch(e){}
  if(!matricula||!cloudSession?.token||byId('v6757SportsProfileModal')?.classList.contains('show'))return false;
  v6757OpenSportsProfile(matricula,{fromRestore:true});return true;
}
window.addEventListener('popstate',()=>{
  if(v6851SportsProfileHistoryArmed && byId('v6757SportsProfileModal')?.classList.contains('show')){
    v6851SportsProfileHistoryArmed=false;v6757CloseSportsProfile(true);
  }
});
// A sessão cloud pode terminar de restaurar depois do DOM. Tentativas curtas bastam no reload.
window.addEventListener('pageshow',()=>{[80,250,600,1200,2200].forEach(ms=>setTimeout(v6851RestoreSportsProfile,ms))});
...
function v6757CloseSportsProfile(fromNativeBack=false){
  if(v6851SportsProfileClosing)return;
  v6851SportsProfileClosing=true;
  const modal=byId('v6757SportsProfileModal');
  if(modal){modal.classList.remove('show');modal.setAttribute('aria-hidden','true')}
  document.body.classList.remove('v6757-profile-open');
  v6851ClearSportsProfile();v6851SportsProfileMatricula='';
  try{v6851SportsProfileWatcher?.destroy?.()}catch(e){}v6851SportsProfileWatcher=null;
  if(!fromNativeBack && v6851SportsProfileHistoryArmed){
    v6851SportsProfileHistoryArmed=false;
    try{history.back()}catch(e){}
  }else v6851SportsProfileHistoryArmed=false;
  setTimeout(()=>{v6851SportsProfileClosing=false;v67610ArmCloseWatcher?.()},0);
}
...
async function v6757OpenSportsProfile(matricula,opts={}){
  ...
  v6851SportsProfileMatricula=String(matricula||'').trim();
  v6851SaveSportsProfile(v6851SportsProfileMatricula);
  modal.classList.add('show');modal.setAttribute('aria-hidden','false');document.body.classList.add('v6757-profile-open');
  // Um estado real no histórico impede a seta/gesto Voltar do Android de fechar o PWA.
  if(!opts?.fromRestore && !v6851SportsProfileHistoryArmed){
    try{history.pushState({...history.state,t2SportsProfile:true},'',location.href);v6851SportsProfileHistoryArmed=true}catch(e){}
  }else if(opts?.fromRestore){
    try{history.pushState({...history.state,t2SportsProfile:true},'',location.href);v6851SportsProfileHistoryArmed=true}catch(e){}
  }
  v6851ArmSportsProfileBack();
  ...
  try{
    ...
    const ownProfile=v6851IsOwnProfile(matricula);
    const ownLocalPhoto=ownProfile?v6717ProfilePhoto():'';
    const photo=v6757ProfilePhoto(ownLocalPhoto||s.foto_perfil);
    const avatarCore=photo?`<div class="v6757-profile-avatar has-photo"><img src="${v6738FeedEsc(photo)}" alt="Foto de ${v6738FeedEsc(s.nome||'militar')}"></div>`:`<div class="v6757-profile-avatar">${v6738FeedInitials(s.nome)}</div>`;
    const avatar=ownProfile?`<div class="v6851-sports-avatar-edit" onclick="document.getElementById('v6851SportsPhotoInput')?.click()" role="button" tabindex="0" aria-label="Trocar foto do perfil">${avatarCore}<span>📷 Trocar foto</span><input id="v6851SportsPhotoInput" type="file" accept="image/*" hidden onchange="v6717ChooseProfilePhoto(this,true)"></div>`:avatarCore;
...
/* ===== V67.68.55 — CENTRAL ADMINISTRATIVA + VOLTAR ANDROID ===== */
let admin52ValidationsCache=[];
let admin52RankingCache=[];
function admin52El(id){return document.getElementById(id)}
const ADMIN54_TAB_KEY='t2_admin_tab_v6855';
let admin54Tab='dashboard';
let admin54TabHistoryArmed=false;
function admin54GetCurrentTab(){return admin54Tab||'dashboard'}
function admin54SaveTab(tab){try{sessionStorage.setItem(ADMIN54_TAB_KEY,tab)}catch(e){}}
function admin54ReadTab(){try{return sessionStorage.getItem(ADMIN54_TAB_KEY)||'dashboard'}catch(e){return 'dashboard'}}
function admin54ValidTab(tab){return ['dashboard','militares','validacoes','ranking','taf','seguranca'].includes(tab)?tab:'dashboard'}
function admin54BackToDashboard(){
  admin54TabHistoryArmed=false;
  admin52ShowTab('dashboard',null,{fromBack:true,noHistory:true});
}
function admin52ShowTab(tab,btn,opts={}){
  tab=admin54ValidTab(tab);
  const previous=admin54Tab;
  admin54Tab=tab;admin54SaveTab(tab);
  // Cria uma etapa real no histórico para a seta/gesto Voltar do Android.
  if(tab!=='dashboard' && previous==='dashboard' && !opts.noHistory && !admin54TabHistoryArmed){
    try{history.pushState({...history.state,t2AdminTab:true,adminTab:tab},'',location.href);admin54TabHistoryArmed=true}catch(e){}
  }
  if(tab==='dashboard')admin54TabHistoryArmed=false;
  const map={dashboard:'admin52Dashboard',militares:'admin52MilitaryBlock',validacoes:'admin52Validacoes',ranking:'admin52Ranking',taf:'admin52TafBlock',seguranca:'admin52SecurityBlock'};
  Object.values(map).forEach(id=>{const el=admin52El(id);if(el)el.hidden=id!==map[tab]});
  const service=admin52El('admin52ServiceBlock');
  if(service){service.hidden=tab!=='validacoes';if(tab==='validacoes'){const host=admin52El('admin52Validacoes');if(host&&service.parentNode!==host)host.appendChild(service)}}
  document.querySelectorAll('.admin52-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.admin52Tab===tab));
  if(tab==='dashboard')admin52RefreshDashboard();
  if(tab==='validacoes'){adminLoadServiceConfig();admin52LoadValidations();}
  if(tab==='ranking')admin52LoadRankingAudit();
  if(tab==='taf'&&typeof officialTafAdminLoad==='function')setTimeout(()=>officialTafAdminLoad(),30);
}
function admin52RenderMetrics(){
  const box=admin52El('admin52Metrics');if(!box)return;
  const total=adminMilitaresCache.length, active=adminMilitaresCache.filter(x=>x.ativo===true).length;
  const vals=admin52ValidationsCache.length, points=admin52RankingCache.reduce((a,r)=>a+Number(r.treinos_validos||0),0);
  box.innerHTML=`<div class="admin52-metric"><b>${total}</b><span>Militares</span></div><div class="admin52-metric"><b>${active}</b><span>Ativos</span></div><div class="admin52-metric"><b>${vals}</b><span>Validações 7d</span></div><div class="admin52-metric"><b>${points}</b><span>Pontos no mês</span></div>`;
}
async function admin52LoadValidations(){
  if(!adminSessionToken)return;
  try{const res=await cloudApi('/rest/v1/rpc/admin_listar_validacoes_servico',{method:'POST',body:JSON.stringify({p_token:adminSessionToken})});const d=await res.json().catch(()=>[]);if(res.ok)admin52ValidationsCache=Array.isArray(d)?d:[];}catch(e){console.warn(e)}
  admin52RenderMetrics();await adminLoadServiceValidations();
}
async function admin52GetRanking(){
  if(!cloudSession?.token||!navigator.onLine)return [];
  const now=new Date(), month=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  try{admin52RankingCache=await v55RpcAll('ranking_constancia',{p_token_militar:cloudSession.token,p_mes:month});}catch(e){console.warn(e);admin52RankingCache=[]}
  return admin52RankingCache;
}
function admin52RankingRows(rows,limit){
  const data=(rows||[]).slice(0,limit||999);if(!data.length)return '<div class="admin-empty">Nenhum ponto registrado neste mês.</div>';
  return data.map((r,i)=>`<div class="admin52-rank-row"><span class="admin52-rank-pos">${i+1}º</span><div><b>${adminEscape(r.graduacao||'BM')} ${adminEscape(r.nome||'Militar')}</b><small>Registro consolidado pelo Ranking de Frequência</small></div><strong>${Number(r.treinos_validos||0)} pt${Number(r.treinos_validos||0)===1?'':'s'}</strong></div>`).join('');
}
async function admin52LoadRankingAudit(){
  const box=admin52El('admin52RankingAudit');if(box)box.innerHTML='<div class="admin-loading">Atualizando ranking…</div>';
  const rows=await admin52GetRanking();
  const now=new Date();const title=admin52El('admin52RankingTitle');if(title)title.textContent=now.toLocaleDateString('pt-BR',{month:'long',year:'numeric'});
  if(box)box.innerHTML=admin52RankingRows(rows);
  admin52RenderMetrics();
}
async function admin52RefreshDashboard(){
  if(!adminSessionToken)return;
  const preview=admin52El('admin52RankingPreview');if(preview)preview.innerHTML='<h3>Ranking do mês</h3><div class="admin-loading">Atualizando…</div>';
  if(!adminMilitaresCache.length)await adminLoadMilitares();
  try{const res=await cloudApi('/rest/v1/rpc/admin_listar_validacoes_servico',{method:'POST',body:JSON.stringify({p_token:adminSessionToken})});const d=await res.json().catch(()=>[]);if(res.ok)admin52ValidationsCache=Array.isArray(d)?d:[];}catch(e){}
  const rows=await admin52GetRanking();admin52RenderMetrics();
  if(preview)preview.innerHTML=`<div class="admin52-card-head"><div><span class="eyebrow">RANKING DE FREQUÊNCIA</span><h3>Resumo do mês</h3></div><button type="button" onclick="admin52ShowTab('ranking')">VER AUDITORIA</button></div>${admin52RankingRows(rows,5)}`;
}
const admin52OldOpenAdmin=openAdmin;
openAdmin=function(){
  admin52OldOpenAdmin();
  if(adminSessionToken)setTimeout(()=>{
    const restoring=(typeof navigationLoadType==='function'&&navigationLoadType()==='reload');
    const tab=restoring?admin54ValidTab(admin54ReadTab()):'dashboard';
    admin54Tab=tab;
    admin52ShowTab(tab,null,{noHistory:true});
    // No reload de uma subseção, arma uma etapa para o Voltar retornar ao Dashboard sem sair do app.
    if(restoring&&tab!=='dashboard'&&!admin54TabHistoryArmed){
      try{history.pushState({...history.state,t2AdminTab:true,adminTab:tab},'',location.href);admin54TabHistoryArmed=true}catch(e){}
    }
  },80)
};
const admin52OldLogin=adminLogin;
adminLogin=async function(){await admin52OldLogin();if(adminSessionToken)setTimeout(()=>{admin54Tab='dashboard';admin52ShowTab('dashboard',null,{noHistory:true})},80)};

/* ===== V67.68.59 — GESTÃO DE MILITARES PRO ===== */
let admin56StatusFilter='all';
let admin56Sort='name';
let admin56DetailCache=new Map();

function admin56EnsureMilitaryUI(){
  const search=document.getElementById('adminSearch');
  if(!search||document.getElementById('admin56MilitaryTools'))return;
  search.placeholder='Buscar por nome ou graduação';
  const tools=document.createElement('div');
  tools.id='admin56MilitaryTools';tools.className='admin56-military-tools';
  tools.innerHTML=`<select id="admin56StatusFilter" onchange="admin56StatusFilter=this.value;adminRenderMilitares()" aria-label="Filtrar militares"><option value="all">Todos</option><option value="active">Ativos</option><option value="inactive">Inativos</option></select><select id="admin56Sort" onchange="admin56Sort=this.value;adminRenderMilitares()" aria-label="Ordenar militares"><option value="name">Nome A–Z</option><option value="rank">Graduação</option><option value="status">Situação</option></select>`;
  search.insertAdjacentElement('afterend',tools);
  if(!document.getElementById('admin56MilitarySummary')){
    const sum=document.createElement('div');sum.id='admin56MilitarySummary';sum.className='admin56-military-summary';
    const card=search.closest('.card');card?.insertBefore(sum,search);
  }
  if(!document.getElementById('admin56MilitaryDetail')){
    const overlay=document.createElement('div');overlay.id='admin56MilitaryDetail';overlay.className='admin56-detail-overlay';overlay.hidden=true;
    overlay.onclick=e=>{if(e.target===overlay)admin56CloseDetail()};
    overlay.innerHTML='<div class="admin56-detail-sheet" role="dialog" aria-modal="true"><div class="admin56-detail-head"><div><span class="eyebrow">GESTÃO DE MILITARES</span><h3>Resumo individual</h3></div><button type="button" onclick="admin56CloseDetail()" aria-label="Fechar">×</button></div><div id="admin56DetailBody"></div></div>';
    document.body.appendChild(overlay);
  }
}
function admin56RankOrder(g){
  const x=String(g||'').toUpperCase();
  const order=['CEL','TC','MAJ','CAP','1º TEN','2º TEN','ST','1º SGT','2º SGT','3º SGT','CB','SD'];
  const i=order.findIndex(v=>x.includes(v));return i<0?99:i;
}
function admin56RenderSummary(rows){
  const box=document.getElementById('admin56MilitarySummary');if(!box)return;
  const total=adminMilitaresCache.length,active=adminMilitaresCache.filter(x=>x.ativo===true).length,inactive=total-active;
  box.innerHTML=`<div><b>${total}</b><span>Cadastrados</span></div><div><b>${active}</b><span>Ativos</span></div><div><b>${inactive}</b><span>Inativos</span></div><div><b>${rows.length}</b><span>Exibidos</span></div>`;
}
adminRenderMilitares=function(){
  admin56EnsureMilitaryUI();
  const list=document.getElementById('adminMilitaryList');
  const search=String(document.getElementById('adminSearch')?.value||'').toLowerCase().trim();
  if(!list)return;
  let rows=adminMilitaresCache.filter(m=>{
    const hay=`${m.nome||''} ${m.graduacao||''}`.toLowerCase();
    const statusOk=admin56StatusFilter==='all'||(admin56StatusFilter==='active'&&m.ativo===true)||(admin56StatusFilter==='inactive'&&m.ativo!==true);
    return statusOk&&(!search||hay.includes(search));
  });
  rows=rows.slice().sort((a,b)=>{
    if(admin56Sort==='status')return Number(b.ativo===true)-Number(a.ativo===true)||String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR');
    if(admin56Sort==='rank')return admin56RankOrder(a.graduacao)-admin56RankOrder(b.graduacao)||String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR');
    return String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR');
  });
  const count=document.getElementById('adminMilitaryCount');if(count)count.textContent=`${adminMilitaresCache.length} militar${adminMilitaresCache.length===1?'':'es'}`;
  admin56RenderSummary(rows);
  if(!rows.length){list.innerHTML='<div class="admin-empty">Nenhum militar encontrado com estes filtros.</div>';return}
  list.innerHTML=rows.map(m=>`<div class="admin-military-row admin56-military-row ${m.ativo?'':'is-inactive'}"><button class="admin56-military-main" type="button" onclick="admin56OpenDetail('${adminEscape(m.matricula)}')"><span class="admin56-avatar">${v6738FeedInitials(m.nome||'BM')}</span><span><b>${adminEscape(m.graduacao||'BM')} ${adminEscape(m.nome||'Sem nome')}</b><small>${m.ativo?'● ATIVO':'○ INATIVO'} • Toque para ver resumo</small></span><span class="admin56-chevron">›</span></button><div class="admin-military-actions"><button class="admin56-profile" type="button" onclick="v6757OpenSportsProfile('${adminEscape(m.matricula)}')">🏅 PERFIL</button><button class="admin-edit" type="button" onclick="adminOpenEditMilitary('${adminEscape(m.matricula)}')">✏️ EDITAR</button><button class="${m.ativo?'admin-disable':'admin-enable'}" type="button" onclick="adminToggleMilitary('${adminEscape(m.matricula)}',${!m.ativo})">${m.ativo?'DESATIVAR':'ATIVAR'}</button></div></div>`).join('');
};
function admin56CloseDetail(){const o=document.getElementById('admin56MilitaryDetail');if(o)o.hidden=true;document.body.classList.remove('admin56-detail-open')}
function admin56FmtDate(v){const d=new Date(v||'');return Number.isFinite(d.getTime())?d.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}):'Sem atividade registrada'}
async function admin56OpenDetail(matricula){
  const m=adminMilitaresCache.find(x=>String(x.matricula||'')===String(matricula||''));if(!m)return;
  admin56EnsureMilitaryUI();const o=document.getElementById('admin56MilitaryDetail'),body=document.getElementById('admin56DetailBody');if(!o||!body)return;
  o.hidden=false;document.body.classList.add('admin56-detail-open');
  body.innerHTML=`<div class="admin56-detail-person"><div class="admin56-avatar big">${v6738FeedInitials(m.nome||'BM')}</div><div><b>${adminEscape(m.graduacao||'BM')} ${adminEscape(m.nome||'Militar')}</b><span class="${m.ativo?'is-active':'is-inactive'}">${m.ativo?'● Cadastro ativo':'○ Cadastro inativo'}</span></div></div><div class="admin-loading">Carregando desempenho esportivo…</div>`;
  try{
    let data=admin56DetailCache.get(String(matricula));
    if(!data){
      const [summaryRows,acts]=await Promise.all([v55RpcAll('perfil_esportivo_resumo_v6757',{p_token_militar:cloudSession.token,p_matricula_perfil:String(matricula)}),v55RpcAll('perfil_esportivo_atividades_v6757',{p_token_militar:cloudSession.token,p_matricula_perfil:String(matricula),p_limite:1})]);
      data={s:summaryRows?.[0]||{},last:Array.isArray(acts)?acts[0]:null};admin56DetailCache.set(String(matricula),data);
    }
    const s=data.s||{},last=data.last;
    body.innerHTML=`<div class="admin56-detail-person"><div class="admin56-avatar big">${v6738FeedInitials(m.nome||'BM')}</div><div><b>${adminEscape(m.graduacao||'BM')} ${adminEscape(m.nome||'Militar')}</b><span class="${m.ativo?'is-active':'is-inactive'}">${m.ativo?'● Cadastro ativo':'○ Cadastro inativo'}</span></div></div><div class="admin56-detail-metrics"><div><b>${Number(s.atividades_total)||0}</b><span>Atividades</span></div><div><b>${Number(s.musculacao)||0}</b><span>Treinos</span></div><div><b>${Number(s.cardio)||0}</b><span>Cardio</span></div><div><b>${v6757FmtMinutes(s.minutos_total)}</b><span>Tempo total</span></div></div><div class="admin56-last"><span class="eyebrow">ÚLTIMA ATIVIDADE</span><b>${last?adminEscape(last.plano||'Atividade física'):'Nenhuma atividade encontrada'}</b><small>${last?admin56FmtDate(last.data_atividade):'Ainda não há atividade disponível no histórico.'}</small></div><div class="admin56-detail-actions"><button type="button" onclick="admin56CloseDetail();v6757OpenSportsProfile('${adminEscape(m.matricula)}')">🏅 ABRIR PERFIL ESPORTIVO</button><button type="button" onclick="admin56CloseDetail();adminOpenEditMilitary('${adminEscape(m.matricula)}')">✏️ EDITAR CADASTRO</button><button type="button" class="${m.ativo?'danger':'success'}" onclick="admin56CloseDetail();adminToggleMilitary('${adminEscape(m.matricula)}',${!m.ativo})">${m.ativo?'DESATIVAR ACESSO':'ATIVAR ACESSO'}</button></div>`;
  }catch(e){console.warn(e);body.insertAdjacentHTML('beforeend','<div class="admin-empty">Não foi possível carregar o desempenho agora. O cadastro administrativo continua disponível.</div>')}
}
const admin56OldLoad=adminLoadMilitares;
adminLoadMilitares=async function(){const r=await admin56OldLoad();admin56EnsureMilitaryUI();adminRenderMilitares();return r};


/* ===== V67.68.59 — AUDITORIA DE VALIDAÇÕES ===== */
function admin59Norm(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase()}
function admin59ValidationDate(v){const d=new Date(v);return Number.isNaN(d.getTime())?'—':d.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'})}
function admin59RenderValidations(){
  const host=document.getElementById('admin59ValidationList'),metrics=document.getElementById('admin59ValidationMetrics');if(!host)return;
  const q=admin59Norm(document.getElementById('admin59ValidationSearch')?.value),status=document.getElementById('admin59ValidationStatus')?.value||'all',days=Number(document.getElementById('admin59ValidationPeriod')?.value||7);
  const all=Array.isArray(admin52ValidationsCache)?admin52ValidationsCache:[];const now=Date.now(),cut=now-days*86400000;
  const filtered=all.filter(v=>{const t=new Date(v.validado_em).getTime();const active=v.ativo===true;return (!q||admin59Norm(`${v.graduacao||''} ${v.nome||''}`).includes(q))&&(status==='all'||(status==='active'&&active)||(status==='expired'&&!active))&&(!Number.isFinite(t)||t>=cut)});
  const active=all.filter(v=>v.ativo===true).length,expired=all.length-active;
  if(metrics)metrics.innerHTML=`<div class="admin59-vmetric"><b>${all.length}</b><span>7 dias</span></div><div class="admin59-vmetric"><b>${active}</b><span>Ativas</span></div><div class="admin59-vmetric"><b>${expired}</b><span>Encerradas</span></div>`;
  if(!filtered.length){host.innerHTML='<div class="admin59-vempty">Nenhuma validação encontrada para os filtros selecionados.</div>';return}
  host.innerHTML=filtered.map(v=>{const act=v.ativo===true;const dist=Number(v.distancia_m);const method=(v.metodo||v.origem||v.tipo_validacao||'QR + GPS');return `<div class="admin59-vrow ${act?'active':'expired'}">
    <div class="admin59-vtop"><div><b>${adminEscape(v.graduacao||'BM')} ${adminEscape(v.nome||'Militar')}</b><small>${act?adminEscape(adminServiceRemaining(v.expira_em)):'Janela operacional encerrada'}</small></div><span class="admin59-vbadge ${act?'active':'expired'}">${act?'ATIVA':'ENCERRADA'}</span></div>
    <div class="admin59-vdetails"><div class="admin59-vdetail"><span>Validado em</span><b>${admin59ValidationDate(v.validado_em)}</b></div><div class="admin59-vdetail"><span>Janela até</span><b>${admin59ValidationDate(v.expira_em)}</b></div><div class="admin59-vdetail"><span>Método</span><b>${adminEscape(method)}</b></div><div class="admin59-vdetail"><span>Distância registrada</span><b>${Number.isFinite(dist)?Math.round(dist)+' m':'—'}</b></div></div>
    <div class="admin59-vnote">A pontuação continua sendo calculada pelas regras oficiais do Ranking. Esta tela é somente para conferência administrativa.</div></div>`}).join('');
}
const admin59OldLoadValidations=admin52LoadValidations;
admin52LoadValidations=async function(){await admin59OldLoadValidations();admin59RenderValidations()};


/* ===== V67.68.60 — AUDITORIA DE PONTUAÇÃO DO RANKING ===== */
function admin60EnsureAudit(){
 if(document.getElementById('admin60RankAudit'))return;
 const o=document.createElement('div');o.id='admin60RankAudit';o.className='admin60-audit-overlay';o.hidden=true;
 o.onclick=e=>{if(e.target===o)admin60CloseAudit()};
 o.innerHTML='<div class="admin60-audit-sheet" role="dialog" aria-modal="true"><div class="admin60-audit-head"><div><span class="eyebrow">AUDITORIA DO RANKING</span><h3>Origem da pontuação</h3></div><button type="button" onclick="admin60CloseAudit()">×</button></div><div id="admin60AuditBody"></div></div>';
 document.body.appendChild(o);
}
function admin60CloseAudit(){const o=document.getElementById('admin60RankAudit');if(o)o.hidden=true;document.body.classList.remove('admin56-detail-open')}
function admin60ActivityDate(a){return new Date(a.data_atividade||a.criado_em||a.created_at||a.date||0)}
function admin60Category(a){const raw=String(a.categoria||a.tipo||a.workout_type||a.plano||'').toLowerCase();if(raw.includes('cardio')||raw.includes('corrida')||raw.includes('esteira')||raw.includes('bike')||raw.includes('remo'))return 'Cardio';if(raw.includes('core'))return 'Core';return 'Treino de força'}
function admin60PointReason(a){const cat=admin60Category(a);const dur=Number(a.duracao||a.duration||0);if(cat==='Core')return {ok:false,text:'Core é complementar e não gera ponto no Ranking.'};if(cat==='Cardio'&&dur<30)return {ok:false,text:'Cardio abaixo de 30 minutos: registrado, mas sem ponto.'};return {ok:true,text:cat==='Cardio'?'Cardio válido (30 min ou mais) durante serviço validado.':'Treino de força válido durante serviço validado.'}}
async function admin60OpenRankAudit(matricula){
 admin60EnsureAudit();const o=document.getElementById('admin60RankAudit'),body=document.getElementById('admin60AuditBody');if(!o||!body)return;
 const row=admin52RankingCache.find(x=>String(x.matricula||x.matricula_militar||'')===String(matricula||''));
 const mil=adminMilitaresCache.find(x=>String(x.matricula||'')===String(matricula||''));
 const name=`${row?.graduacao||mil?.graduacao||'BM'} ${row?.nome||mil?.nome||'Militar'}`,points=Number(row?.treinos_validos||0);
 o.hidden=false;document.body.classList.add('admin56-detail-open');body.innerHTML=`<div class="admin60-person"><b>${adminEscape(name)}</b><span>${points} ponto${points===1?'':'s'} consolidado${points===1?'':'s'} no mês atual</span></div><div class="admin-loading">Carregando atividades que compõem a auditoria…</div>`;
 try{
  const acts=await v55RpcAll('perfil_esportivo_atividades_v6757',{p_token_militar:cloudSession.token,p_matricula_perfil:String(matricula),p_limite:100});
  const now=new Date(),start=new Date(now.getFullYear(),now.getMonth(),1),end=new Date(now.getFullYear(),now.getMonth()+1,1);
  const month=(Array.isArray(acts)?acts:[]).filter(a=>{const d=admin60ActivityDate(a);return d>=start&&d<end});
  const candidates=month.filter(a=>a.serviceDay===true||a.service_day===true||a.em_servico===true||a.servico_validado===true||String(a.serviceDay)==='true');
  const src=candidates.length?candidates:month;
  const cards=src.map(a=>{const r=admin60PointReason(a),d=admin60ActivityDate(a);return `<div class="admin60-point ${r.ok?'':'no-point'}"><div class="admin60-point-top"><b>${adminEscape(a.plano||admin60Category(a))}</b><strong>${r.ok?'ELEGÍVEL':'SEM PONTO'}</strong></div><small>${Number.isFinite(d.getTime())?d.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'Data não disponível'} • ${adminEscape(admin60Category(a))}</small><small>${adminEscape(r.text)}</small></div>`}).join('');
  body.innerHTML=`<div class="admin60-person"><b>${adminEscape(name)}</b><span>${points} ponto${points===1?'':'s'} consolidado${points===1?'':'s'} no mês atual</span></div>${cards||'<div class="admin60-empty">Nenhuma atividade do mês foi encontrada no histórico esportivo deste militar.</div>'}<div class="admin60-rule"><b>Conferência administrativa</b><br>O total oficial continua sendo o valor consolidado pelo Ranking no banco. Esta visão mostra as atividades disponíveis no histórico para explicar a composição: Treino de Força ou Cardio válido podem gerar ponto durante presença validada; Cardio exige 30 minutos; Core não pontua; e a janela operacional respeita o teto definido pelo Ranking.</div>`;
 }catch(e){console.warn(e);body.insertAdjacentHTML('beforeend','<div class="admin60-empty">Não foi possível carregar o detalhamento agora. O total oficial do Ranking permanece inalterado.</div>')}
}
admin52RankingRows=function(rows,limit){
 const data=(rows||[]).slice(0,limit||999);if(!data.length)return '<div class="admin-empty">Nenhum ponto registrado neste mês.</div>';
 return data.map((r,i)=>{const mat=String(r.matricula||r.matricula_militar||'');return `<div class="admin52-rank-row admin60-rank-row" ${mat?`onclick="admin60OpenRankAudit('${adminEscape(mat)}')"`:''}><span class="admin52-rank-pos">${i+1}º</span><div><b>${adminEscape(r.graduacao||'BM')} ${adminEscape(r.nome||'Militar')}</b><small>${mat?'Toque para auditar a origem dos pontos':'Registro consolidado pelo Ranking de Frequência'}</small></div><strong>${Number(r.treinos_validos||0)} pt${Number(r.treinos_validos||0)===1?'':'s'}</strong></div>`}).join('');
};
