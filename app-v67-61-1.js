...
const V67581_MOTIVATION_BOOT_KEY='t2_v67611_motivation_boot';
...
/* ===== V67.61.0 — EXPERIÊNCIA VISUAL DO TREINO ===== */
function v67610UpdateWorkoutVisuals(){
  if(!currentExercise)return;
  const totalSets=Math.max(1,Number(currentExercise.sets)||1);
  const setNo=Math.max(1,Math.min(Number(currentSet)||1,totalSets));
  const dots=byId('setProgressDots');
  if(dots){
    dots.innerHTML=Array.from({length:totalSets},(_,i)=>`<i class="${i+1<setNo?'done':i+1===setNo?'current':''}"></i>`).join('');
  }
  ...
  if(fill){
    ...
    const setFraction=(setNo-1)/totalSets;
    const pct=activePlanIndex>=0?Math.min(100,Math.max(2,((idx+setFraction)/total)*100)):0;
    fill.style.width=`${pct}%`;
  }
}
function v67610SetRestState(active){
  const card=document.querySelector('.workout-rest-card');
  if(card)card.classList.toggle('is-running',!!active);
  const hint=byId('restHint');
  if(hint)hint.textContent=active?'Próxima série em breve…':'Recupere o fôlego para a próxima série';
}
function openExercise(id,inPlan=false){
  ...
  v67610UpdateWorkoutVisuals();
  v67610SetRestState(false);
...
function skipCurrentSet(){
  ...
  // Série pulada não é registrada no histórico e não conta para estatísticas/ranking.
  // Avança sempre de forma imediata e previsível.
  ...
  if(setNo<totalSets){
    ...
    if(label)label.textContent=`Série ${currentSet} de ${totalSets}`;
    v67610UpdateWorkoutVisuals();
    const weight=byId('weight'), reps=byId('reps');
  ...
  // Se era a última série, pular a série já leva ao próximo exercício.
  // No último exercício, conclui o fluxo do treino.
  v67603SaveActivePlanState();
  saveNavigationState('exercise');
  nextPlanExercise();
}
...
function skipCurrentExercise(){
  if(!currentExercise || activePlanIndex<0)return;
  // Exercício pulado não cria séries nem altera histórico/ranking.
  pauseTimer();
  v67603SaveActivePlanState();
  saveNavigationState('exercise');
  nextPlanExercise();
...
function updateClock(){
  ...
  if(clock)clock.textContent=`${String(Math.floor(timer/60)).padStart(2,'0')}:${String(timer%60).padStart(2,'0')}`;
  const ring=document.querySelector('.rest-ring');
  if(ring){
    const total=Math.max(1,Number(currentExercise?.rest)||90);
    const pct=Math.max(0,Math.min(100,(timer/total)*100));
    ring.style.setProperty('--rest-progress',`${pct*3.6}deg`);
  }
}
function startTimer(){if(tick)return;v67610SetRestState(true);tick=setInterval(()=>{timer=Math.max(0,timer-1);updateClock();if(timer===0){pauseTimer();const hint=byId('restHint');if(hint)hint.textContent='✓ Descanso concluído — próxima série';if(navigator.vibrate)navigator.vibrate([250,100,250])}},1000)}
function pauseTimer(){clearInterval(tick);tick=null;v67610SetRestState(false)}
