...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden'){
    // V67.68.32 — grava o ponto EXATO antes de o Android/PWA suspender a página.
    try{
...
function showView(id){
  // V67.68.32 — no boot/reload, um personalizado ativo tem prioridade absoluta
...
/* ===== V67.68.32 — CHECKPOINT DURÁVEL DO TREINO PERSONALIZADO ===== */
...
// V67.68.32 — trava curta de boot: durante um reload com personalizado ativo,
// rotinas tardias de login/feed não podem vencer a restauração e mandar para Home.
let v676832BootRestoreLock = navigationLoadType()==='reload' && !!v676830ReadCheckpoint();
...
function openExercise(id,inPlan=false){
  ...
  showView('exercise');
  // V67.68.32 — durante restauração, NUNCA zera a série para 1.
...
function restoreNavigationState(force=false){
  // V67.68.32 — restauração do personalizado é LOCAL e não depende de login/cloud.
  ...
  // V67.68.32 — ao ATUALIZAR a página durante um treino estruturado/personalizado,
...
function cloudHideGate(afterLogin=false){
  ...
  // V67.68.32 — revalida após as rotinas assíncronas de boot/login. Isso impede
  // que uma chamada posterior à Home desfaça a tela restaurada.
  if(v676832BootRestoreLock || (navigationLoadType()==='reload' && v676830ReadCheckpoint())){
