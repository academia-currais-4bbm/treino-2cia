...
/* ===== V67.59.5 — NOTIFICAÇÃO INTERNA -> ATIVIDADE DO FEED ===== */
async function v67595OpenNotificationFeed(feedId){
  ...
  const bell=document.querySelector('[aria-controls="feedNotificationsPanel"]');
  if(bell)bell.setAttribute('aria-expanded','false');
  const u=new URL(location.href);u.searchParams.set('feed',String(id));
  history.replaceState(history.state,'',u.pathname+u.search+u.hash);
  await v67594OpenFeedDeepLink();
}

/* ===== V67.59.4 — ABRIR ATIVIDADE DO FEED PELO PUSH ===== */
let v67594DeepLinkRunning=false;
async function v67594OpenFeedDeepLink(){
  if(v67594DeepLinkRunning)return;
  const params=new URLSearchParams(location.search);
  const feedId=Number(params.get('feed')||0);
  if(!Number.isFinite(feedId)||feedId<=0)return;
  if(!cloudSession?.token)return; // será tentado novamente após o login
  v67594DeepLinkRunning=true;
  try{
    if(typeof showView==='function')showView('home');
    if(typeof v6738LoadFeed==='function')await v6738LoadFeed(true);
    let card=document.querySelector(`.sports-feed-card[data-feed-id="${feedId}"]`);
    // Se a atividade estiver fora dos 7 dias visíveis, amplia a janela já carregada.
    if(!card && Array.isArray(v6753FeedRows)){
      const row=v6753FeedRows.find(r=>Number(r?.id)===feedId);
      if(row){
        ...
        v6738RenderFeed(v6753FeedRows);
        card=document.querySelector(`.sports-feed-card[data-feed-id="${feedId}"]`);
      }
    }
    if(card){
    ...
    // Limpa o parâmetro para atualizar/reabrir o app não repetir a navegação.
    const clean=new URL(location.href);clean.searchParams.delete('feed');
    history.replaceState(history.state,'',clean.pathname+clean.search+clean.hash);
  }catch(e){console.warn('Deep link do Feed:',e)}
  finally{v67594DeepLinkRunning=false}
}
window.addEventListener('load',()=>{
  let tries=0;
  const timer=setInterval(()=>{
    tries++;
    if(!new URLSearchParams(location.search).has('feed')){clearInterval(timer);return}
    if(cloudSession?.token){clearInterval(timer);setTimeout(v67594OpenFeedDeepLink,250);return}
