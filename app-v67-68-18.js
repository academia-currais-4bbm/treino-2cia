...
let v676818StartingPlan=false;
function startPlanWorkout(){
  // V67.68.18 — o botão INICIAR usa o MESMO caminho de abertura que já funciona
  // ao tocar diretamente no exercício. A sessão só é persistida depois que a
  // tela do primeiro exercício foi aberta, evitando que o próprio estado salvo
  // interrompa/redirecione o clique de início.
  if(v676818StartingPlan)return true;
  ...
  v676818StartingPlan=true;
  const unlock=()=>setTimeout(()=>{v676818StartingPlan=false},700);
  try{
    ...
    // Sessão de outro treino continua protegida. Uma sessão do mesmo treino
    // deixada por tentativas anteriores não pode bloquear um novo INICIAR.
    if(existing && String(existing.name||'')!==String(activePlanName||'')){
      if(confirm('Já existe outro treino em andamento. Deseja retomá-lo?')){
        const ok=v67604ResumeActivePlan();
        unlock();
        if(ok)return true;
        try{localStorage.removeItem('t2_active_plan')}catch(e){}
    ...
    // Importante: abre primeiro. É exatamente o fluxo comprovadamente funcional
    // usado pelo toque no card do exercício.
    const first=activePlanExercises[0];
    const opened=openExercise(first.id,true);
    if(opened===false)throw new Error('Primeiro exercício não encontrado.');

    // Só depois da tela aberta registramos a sessão ativa.
    currentSet=1;
...
function openExercise(id,inPlan=false){
  ...
  // V67.68.18 — entra na tela de execução IMEDIATAMENTE. Assim nenhuma rotina
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-18.js', {updateViaCache:'none'}));
...
localStorage.setItem('t2_app_version','v67.68.18');
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-18.js', {updateViaCache:'none'});
