...
function toggleSwimTypeFields(){
  ...
  const pool=byId('cardioPoolFields'),loc=byId('cardioSwimLocation'); if(pool)pool.style.display=type==='pool'?'block':'none';if(loc)loc.style.display=type==='open'?'block':'none';
...
function saveCardioWorkout(){
  ...
  const record={
    ...
    activityLocationMode:['run','swim'].includes(machine)?(cardioActivityLocation.mode||null):null,
    activityLocationLabel:['run','swim'].includes(machine)?(String(cardioActivityLocation.label||'').trim()||null):null,
    activityLocationGps:['run','swim'].includes(machine)&&cardioActivityLocation.mode==='gps'?{lat:cardioActivityLocation.lat,lng:cardioActivityLocation.lng,accuracy:cardioActivityLocation.accuracy}:null,
    feedLocation:['run','swim'].includes(machine)?(String(cardioActivityLocation.label||'').trim()||null):undefined,
