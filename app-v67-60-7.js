...
// V67.60.7 — navegação interna robusta para PWA/Android.
// Cada tela interna recebe uma profundidade própria no History API.
// A seta do app e o botão/gesto Voltar do Android passam a consumir a mesma pilha.
...
function v67607HistoryState(viewId,depth){
  return {t2App:true,t2NavVersion:67607,depth:Math.max(0,Number(depth)||0),snapshot:appHistorySnapshot(viewId)};
}
function v67607CurrentDepth(){
  const st=window.history.state;
  return st?.t2App ? Math.max(0,Number(st.depth)||0) : 0;
}
function ensureAppHistoryState(){
  try{
    ...
    const st=window.history.state;
    if(!st?.t2App){
      window.history.replaceState(v67607HistoryState(active,active==='home'?0:1),'',location.href);
    }else if(st.t2NavVersion!==67607 || st?.snapshot?.view!==active){
      window.history.replaceState(v67607HistoryState(active,v67607CurrentDepth()),'',location.href);
    }
  }catch(e){}
}
...
function showView(id){
  ...
  if(!navRestoring && !browserNavHandling && previous && previous!==id){
    try{
      const currentDepth=v67607CurrentDepth();
      window.history.replaceState(v67607HistoryState(previous,currentDepth),'',location.href);
    }catch(e){}
  }
  ...
  if(!navRestoring && !browserNavHandling && previous!==id){
    try{
      const nextDepth=v67607CurrentDepth()+1;
      window.history.pushState(v67607HistoryState(id,nextDepth),'',location.href);
    }catch(e){}
  }
}
...
function appBack(fallbackView='home', fallbackAction=null){
  try{
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    // Só usa history.back quando existe uma entrada INTERNA anterior do app.
    // Não usa history.length, pois no Android/PWA ele também conta entradas externas
    // e podia fechar o aplicativo em vez de voltar para a tela anterior.
    if(st?.t2App && Number(st.depth)>0){
      window.history.replaceState(v67607HistoryState(active,Number(st.depth)),'',location.href);
      window.history.back();
      return;
    }
  }catch(e){}
  ...
  if(fallbackView && fallbackView!==(document.querySelector('.view.active')?.id||'')){
...
window.addEventListener('popstate',(event)=>{
  ...
  try{
    ...
    // Entrada sem estado interno: se ainda há uma tela interna visível,
    // converte a posição atual em Home em vez de deixar o PWA encerrar.
    const active=document.querySelector('.view.active')?.id||'home';
    if(active!=='home'){
      const snap=appHistorySnapshot('home');
      const root=v67607HistoryState('home',0);
      window.history.replaceState(root,'',location.href);
...
// V67.60.7 — após sessão estar disponível, tentativas adicionais são disparadas em pageshow/online.
window.addEventListener('online',()=>setTimeout(()=>v67607RecoverMissingFeedActivities(),500));
window.addEventListener('pageshow',()=>setTimeout(()=>{if(cloudSession?.token)v67607RecoverMissingFeedActivities()},900));
...
/* ===== V67.60.7 — RECUPERAÇÃO HISTÓRICO → FEED =====
...
async function v67382RefreshFeed(button){
  ...
  try{
    v6738FeedLastLoad=0;
    // V67.60.7: primeiro recupera atividades locais com atividadeId que nunca
    // chegaram ao Feed; depois mantém a reconciliação de exclusões já existente.
    await v67607RecoverMissingFeedActivities();
