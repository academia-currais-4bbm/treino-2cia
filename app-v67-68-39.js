...
function v676839RepairWorkoutDurations(list){
  ...
  const out=(Array.isArray(list)?list:[]).map(w=>{
    ...
    if(strength && Number(w.duration||0)<=1 && Number(w.sets||0)>=5 && wall>=5){changed=true;return {...w,duration:wall,durationRepairedV676839:true};}
    return w;
  });
...
function workoutHistory(){
  try{
    ...
    if(fixed.changed){localStorage.setItem('t2_workouts',JSON.stringify(fixed.list));try{if(typeof cloudScheduleSync==='function')cloudScheduleSync()}catch(e){}}
    return fixed.list;
