...
/* ===== V67.59.6 — NOTIFICAÇÕES -> ATIVIDADE DO FEED (CONSUMO ÚNICO) ===== */
let v67596FeedTargetRunning=false;

function v67596ConsumeFeedParam(){
  try{
    const u=new URL(location.href);
    const id=Number(u.searchParams.get('feed')||0)||0;
    if(u.searchParams.has('feed')){
      u.searchParams.delete('feed');
      // Remove o deep link ANTES de qualquer showView/pushState. Assim um refresh,
      // voltar/avançar ou outra navegação não reaproveita a notificação antiga.
...
async function v67596OpenFeedTarget(feedId){
  const id=Number(feedId)||0;
  if(id<=0||v67596FeedTargetRunning||!cloudSession?.token)return false;
  v67596FeedTargetRunning=true;
  try{
  ...
  finally{v67596FeedTargetRunning=false}
}

async function v67595OpenNotificationFeed(feedId){
  const id=Number(feedId)||0;if(id<=0)return;
  const panel=byId('feedNotificationsPanel');
  if(panel)panel.hidden=true;
  const bell=document.querySelector('.sports-feed-notifications');
  if(bell)bell.setAttribute('aria-expanded','false');
  // Clique no sininho é navegação interna: não grava ?feed na URL.
  await v67596OpenFeedTarget(id);
}

// Compatibilidade com chamadas já existentes. O parâmetro é consumido uma única vez.
async function v67594OpenFeedDeepLink(){
  const id=v67596ConsumeFeedParam();
  if(id<=0)return false;
  if(!cloudSession?.token){
    try{sessionStorage.setItem('v67596PendingFeedId',String(id))}catch(e){}
    return false;
  }
  return v67596OpenFeedTarget(id);
}

window.addEventListener('load',()=>{
  // Consome imediatamente para que o endereço fique limpo antes da navegação interna.
  let id=v67596ConsumeFeedParam();
  if(id>0){try{sessionStorage.setItem('v67596PendingFeedId',String(id))}catch(e){}}
  let tries=0;
  const timer=setInterval(()=>{
    tries++;
    let pending=0;
    try{pending=Number(sessionStorage.getItem('v67596PendingFeedId')||0)||0}catch(e){}
    if(pending<=0){clearInterval(timer);return}
    if(cloudSession?.token){
      clearInterval(timer);
      try{sessionStorage.removeItem('v67596PendingFeedId')}catch(e){}
      setTimeout(()=>v67596OpenFeedTarget(pending),250);
      return;
    }
