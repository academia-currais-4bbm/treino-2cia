...
/* ===== V67.67.0 — LOCALIZAÇÃO UNIVERSAL DA ATIVIDADE ===== */
let v67670ActivityLocation={mode:'',label:'',lat:null,lng:null,accuracy:null};
function v67670ResetActivityLocation(){v67670ActivityLocation={mode:'',label:'',lat:null,lng:null,accuracy:null};v67670RenderLocationHosts()}
function v67670LocationCard(){
  const v=v67670ActivityLocation;
  return `<div class="activity-location-card v67670-location-card"><b>📍 Local da atividade <small class="field-optional">opcional</small></b><p>Se ainda estiver no local, use o GPS. Se já tiver saído, informe manualmente onde realizou a atividade.</p><div class="location-actions"><button type="button" onclick="v67670CaptureLocation()">USAR LOCALIZAÇÃO ATUAL</button><button type="button" onclick="v67670ChooseManualLocation()">INFORMAR LOCAL</button></div><input class="v67670-location-input" type="text" maxlength="80" placeholder="Ex.: Academia da 2ª CIA / Pista / Piscina" value="${String(v.label||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;')}" oninput="v67670LocationInput(this.value)"><div class="activity-location-status">${v.label?(v.mode==='gps'?'✓ GPS capturado • informe/confirme o nome do local.':'✓ Local informado manualmente.'):'Nenhuma localização adicionada.'}</div></div>`;
}
...
function v67670LocationInput(value){v67670ActivityLocation.label=String(value||'').trim().slice(0,80);if(!v67670ActivityLocation.mode)v67670ActivityLocation.mode='manual';v67670RenderLocationHosts()}
function v67670ChooseManualLocation(){v67670ActivityLocation.mode='manual';v67670RenderLocationHosts();setTimeout(()=>document.querySelector('.view.active .v67670-location-input')?.focus(),0)}
async function v67670CaptureLocation(){
  if(!navigator.geolocation){alert('Geolocalização indisponível neste aparelho.');return}
  try{const pos=await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:true,timeout:20000,maximumAge:0}));const c=pos.coords;v67670ActivityLocation={mode:'gps',label:v67670ActivityLocation.label||'',lat:Number(c.latitude.toFixed(5)),lng:Number(c.longitude.toFixed(5)),accuracy:Math.round(c.accuracy||0)};v67670RenderLocationHosts();setTimeout(()=>document.querySelector('.view.active .v67670-location-input')?.focus(),0)}catch(e){alert('Não foi possível obter sua localização. Verifique a permissão de GPS e tente novamente.')}
}
