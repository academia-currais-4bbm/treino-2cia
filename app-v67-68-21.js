...
function startPlanWorkout(){
  // V67.68.21 — início resiliente: a abertura visual do exercício é o marco de
...
function nextPlanExercise(){
  ...
  if(activePlanIndex < activePlanExercises.length-1){
    ...
    // V67.68.21 — zera também o estado visual/persistido. Reaplica após o frame
    // para impedir que um estado de navegação do exercício anterior restaure 4/4.
...
function isBodyweightExercise(ex){
  ...
  // V67.68.21 — exercícios explicitamente sem carga externa.
  // Barra fixa e flexão de braço nunca exibem/solicitam Carga (kg), inclusive variações do nome.
  if(/(^| )barra fixa( |$)/.test(name) || /(^| )flexao de? braco?s?( |$)/.test(name) || /^flexao( |$)/.test(name)) return true;
...
function openExercise(id,inPlan=false){
  ...
  // V67.68.21 — entra na tela de execução IMEDIATAMENTE. Assim nenhuma rotina
...
function completeSet(){
  ...
  if(currentSet<currentExercise.sets){
    ...
    // V67.68.21 — a última série também conta visualmente como concluída e
...
localStorage.setItem('t2_app_version','v67.68.21');
