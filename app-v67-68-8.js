...
function startPlanWorkout(){
  ...
  if(existing){
    // V67.68.8 — um estado ativo sem nenhuma série realmente concluída é apenas
    // uma tentativa de início interrompida. Não deve bloquear um novo treino.
    // Isso corrige o "treino fantasma" criado quando a abertura da execução
    // falhava antes de a primeira série ser registrada.
    let existingHasProgress=false;
    try{
      const startedMs=new Date(existing.startedAt).getTime();
      existingHasProgress=history().some(x=>{
        const t=new Date(x?.date).getTime();
    ...
    if(!existingHasProgress){
    ...
    }else if(v67687CanResumeActivePlan(existing)){
      if(confirm('Já existe um treino com séries registradas em andamento. Deseja retomá-lo?')){
        if(v67604ResumeActivePlan()){
      ...
      // Estado órfão/incompatível (ex.: treino personalizado editado): não entra
      // em loop de confirmação. Descarta somente o treino ativo, sem tocar nos
      // treinos personalizados nem no histórico.
      v67687DiscardBrokenActivePlan();
    }
