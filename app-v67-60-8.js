...
// V67.60.8 — navegação PWA/Android com entrada sentinela.
// O Android sempre encontra uma entrada anterior dentro do próprio PWA; assim o
// primeiro Voltar em uma tela interna chega ao popstate do app, em vez de fechar.
...
const V67608_NAV_VERSION=67608;
const V67608_SENTINEL='t2cia-back-sentinel-v67608';
function v67608HistoryState(viewId,depth){
  return {t2App:true,t2NavVersion:V67608_NAV_VERSION,depth:Math.max(0,Number(depth)||0),snapshot:appHistorySnapshot(viewId)};
}
function v67608CurrentDepth(){
  const st=window.history.state;
  return st?.t2App && !st?.t2Sentinel ? Math.max(0,Number(st.depth)||0) : 0;
}
function v67608InstallSentinel(force=false){
  try{
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(!force && st?.t2App && st?.t2NavVersion===V67608_NAV_VERSION && !st?.t2Sentinel)return;
    // A entrada atual vira a sentinela e uma nova entrada representa a tela visível.
    // Portanto o Android nunca está numa tela interna sem ter para onde voltar.
    window.history.replaceState({t2App:true,t2Sentinel:true,t2NavVersion:V67608_NAV_VERSION,key:V67608_SENTINEL},'',location.href);
    window.history.pushState(v67608HistoryState(active,active==='home'?0:1),'',location.href);
  }catch(e){console.warn('Não foi possível instalar a proteção do botão Voltar:',e)}
}
function ensureAppHistoryState(){
  try{
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(!st?.t2App || st?.t2Sentinel || st?.t2NavVersion!==V67608_NAV_VERSION){
      v67608InstallSentinel(true);
    }else if(st?.snapshot?.view!==active){
      window.history.replaceState(v67608HistoryState(active,v67608CurrentDepth()),'',location.href);
    }
  }catch(e){}
}
...
function showView(id){
  ...
  if(!navRestoring && !browserNavHandling && previous && previous!==id){
    try{
      ...
      window.history.replaceState(v67608HistoryState(previous,v67608CurrentDepth()),'',location.href);
    }catch(e){}
  }
  ...
  if(!navRestoring && !browserNavHandling && previous!==id){
    try{
      const nextDepth=v67608CurrentDepth()+1;
      window.history.pushState(v67608HistoryState(id,nextDepth),'',location.href);
    }catch(e){}
  }
}
...
function appBack(fallbackView='home', fallbackAction=null){
  try{
    ...
    const active=document.querySelector('.view.active')?.id||'home';
    const st=window.history.state;
    if(st?.t2App && !st?.t2Sentinel && Number(st.depth)>0){
      window.history.replaceState(v67608HistoryState(active,Number(st.depth)),'',location.href);
      window.history.back();
      return;
    }
  }catch(e){}
...
window.addEventListener('popstate',(event)=>{
  ...
  try{
    ...
    if(st?.t2Sentinel){
      const active=document.querySelector('.view.active')?.id||'home';
      // Se algum fluxo perdeu as entradas intermediárias, a sentinela ainda
      // impede o fechamento: volta para Home e reinstala a proteção.
      if(active!=='home'){
        document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));
        ...
        window.history.pushState(v67608HistoryState('home',0),'',location.href);
        setTimeout(()=>v6738LoadFeed?.(false),60);
        setTimeout(()=>v6748RenderDashboard?.(),20);
        window.scrollTo({top:0,behavior:'auto'});
        return;
      }
      // Já está na Home: o próximo nível é realmente fora do app.
      // Não repõe a sentinela aqui; permite ao Android concluir o Voltar.
    ...
    // Estado externo/legado alcançado a partir de tela interna: não fecha o PWA.
    const active=document.querySelector('.view.active')?.id||'home';
    if(active!=='home'){
      document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));
      byId('home')?.classList.add('active');
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',()=>v67608InstallSentinel(),{once:true});
}else{
  v67608InstallSentinel();
}
...
function restoreNavigationState(force=false){
  ...
  if(!force && !NAV_RESTORE_ON_BOOT){
    ...
    try{
      ...
      window.scrollTo({top:0,behavior:'auto'});
      window.history.replaceState(v67608HistoryState('home',0),'',location.href);
    }catch(e){}
  ...
  try{
    ...
    try{
      ...
      if(activeNow!=='home' && (!currentState?.t2App || currentState?.snapshot?.view!==activeNow)){
        ...
        window.history.pushState(v67608HistoryState(activeNow,Math.max(1,v67608CurrentDepth()+1)),'',location.href);
