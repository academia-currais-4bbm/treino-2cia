...
function v676844PerformanceHub(records){
  ...
  const monthPrs=prs.filter(p=>{const d=new Date(p.date);return !Number.isNaN(d.getTime())&&v676844MonthKey(d)===currentKey});
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-45.js', {updateViaCache:'none'}));
...
localStorage.setItem('t2_app_version','v67.68.45');
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-45.js', {updateViaCache:'none'});
