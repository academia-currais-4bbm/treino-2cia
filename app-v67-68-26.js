...
function nextPlanExercise(){
  ...
  if(activePlanIndex < activePlanExercises.length-1){
    ...
    // V67.68.26 — ao ENTRAR em um novo exercício, o relógio fica no tempo
...
// V67.68.26 — nos personalizados, dispara o descanso APÓS toda a cadeia da série.
...
function v676824StartCustomRestAfterSet(seconds=null){
  ...
  // V67.68.26 — mesmo ciclo dos treinos prontos: cada conclusão/pulo
...
function startTimer(){if(tick)return;tick=setInterval(()=>{timer=Math.max(0,timer-1);updateClock();if(timer===0){pauseTimer();if(navigator.vibrate)navigator.vibrate([250,100,250]);
  // V67.68.26 — nos treinos personalizados, ao terminar o descanso o relógio
...
localStorage.setItem('t2_app_version','v67.68.26');
