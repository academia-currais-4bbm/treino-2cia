...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden'){
    // V67.68.33 — grava o ponto EXATO antes de o Android/PWA suspender a página.
    try{
...
function showView(id){
  // V67.68.33 — no boot/reload, um personalizado ativo tem prioridade absoluta
  // sobre qualquer chamada tardia de Home. Libera assim que a restauração concluir.
  if(id==='home' && v676833BootRestoreLock && !v676833BootRestoreDone){
    setTimeout(()=>{try{v676833EnforceCustomRestore()}catch(e){}},0);
    return;
  }
...
/* ===== V67.68.33 — CHECKPOINT DURÁVEL DO TREINO PERSONALIZADO ===== */
...
// V67.68.33 — trava curta de boot: durante um reload com personalizado ativo,
// rotinas tardias de login/feed não podem vencer a restauração e mandar para Home.
let v676833BootRestoreLock = navigationLoadType()==='reload' && !!v676830ReadCheckpoint();
let v676833BootRestoreDone = false;
function v676833EnforceCustomRestore(){
  const cp=v676830ReadCheckpoint();
  if(!cp||!v676830IsCustomName(cp.name)){v676833BootRestoreLock=false;return false;}
  const ok=v676830RestoreCheckpoint(cp,false);
  if(ok){v676833BootRestoreDone=true;v676833BootRestoreLock=false;}
  return ok;
}
...
function v67604PauseWorkout(goHome=false){
  ...
  if(goHome){
    // V67.68.33 — pausa manual com retorno à Home é intencional. Mantém o progresso
    // para RETOMAR, mas impede a restauração automática de puxar a tela de exercício.
    if(v676830IsCustomName())v676830WriteCheckpoint({status:'pausedHome'});
    showView('home');setTimeout(v67604RenderActiveWorkoutCard,20);
  }
  return true;
}
...
function v67604CancelWorkout(){
  ...
  // V67.68.33 — cancelamento é terminal: não pode existir checkpoint capaz de ressuscitar o treino.
  v676830ClearCheckpoint();
  v676833BootRestoreLock=false; v676833BootRestoreDone=true;
  pauseTimer();
...
async function finishPlanWorkout(){
  ...
  localStorage.removeItem('t2_active_plan');
  // V67.68.33 — treino salvo/concluído: encerra definitivamente a sessão restaurável.
  v676830ClearCheckpoint();
  v676833BootRestoreLock=false; v676833BootRestoreDone=true;
  pauseTimer();
  updateLast();
  ...
  currentExercise=null; currentSet=1;
  // O nome/exercícios só são limpos depois de o resumo já ter sido montado acima.
  activePlanName=''; activePlanExercises=[];
...
function openExercise(id,inPlan=false){
  ...
  showView('exercise');
  // V67.68.33 — durante restauração, NUNCA zera a série para 1.
  // O checkpoint é a fonte de verdade e deve sobreviver ao reload completo.
  const v676833RestoringSet = navRestoring && v676830IsCustomName() ? Math.max(1,Number(currentSet)||1) : 1;
  currentGroup=currentExercise.group;currentSet=v676833RestoringSet;timer=currentExercise.rest;pauseTimer();
...
function v6750ClosePostWorkout(){
  document.getElementById('v6750PostWorkout')?.classList.remove('show');
  // V67.68.33 — CONCLUIR encerra o pós-treino e retorna de fato à página inicial.
  v676830ClearCheckpoint();
  try{localStorage.removeItem('t2_active_plan')}catch(e){}
  showView('home');
}
function v6750OpenFeed(){
  ...
  try{localStorage.removeItem('t2_active_plan')}catch(e){}
  showView('home');
  setTimeout(()=>document.querySelector('.sports-feed-shell')?.scrollIntoView({behavior:'smooth',block:'start'}),180);
}
...
function restoreNavigationState(force=false){
  // V67.68.33 — restauração do personalizado é LOCAL e não depende de login/cloud.
  ...
  // V67.68.33 — ao ATUALIZAR a página durante um treino estruturado/personalizado,
...
function cloudHideGate(afterLogin=false){
  ...
  // V67.68.33 — revalida após as rotinas assíncronas de boot/login. Isso impede
  // que uma chamada posterior à Home desfaça a tela restaurada.
  if(v676833BootRestoreLock || (navigationLoadType()==='reload' && v676830ReadCheckpoint())){
    [40,180,600,1400].forEach(ms=>setTimeout(()=>{try{v676833EnforceCustomRestore()}catch(e){}},ms));
  }
...
document.addEventListener('visibilitychange',()=>{
  ...
  if(document.visibilityState==='visible'){
    const cp=v676830ReadCheckpoint();
    if(cp&&v676830IsCustomName(cp.name)&&cp.status!=='pausedHome'){
