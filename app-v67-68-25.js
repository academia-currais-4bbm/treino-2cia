...
function nextPlanExercise(){
  ...
  if(activePlanIndex < activePlanExercises.length-1){
    // Guarda o descanso do exercício que acabou. openExercise() pausa/resetará o relógio
    // para o novo exercício, então o descanso precisa ser iniciado DEPOIS da transição.
    const transitionRest=Math.max(0,Number(currentExercise?.rest)||0);
    activePlanIndex++;
    pauseTimer();
    currentSet=1;
    ...
    // Descanso entre exercícios: começa automaticamente após a nova tela estar pronta.
    v676823StartRest(transitionRest);
...
// V67.68.25 — nos personalizados, dispara o descanso APÓS toda a cadeia da série.
...
function v676824StartCustomRestAfterSet(seconds=null){
  ...
  const fire=()=>{
    if(sec<=0)return;
    pauseTimer(); timer=sec; updateClock(); startTimer();
  };
  fire();
  setTimeout(fire,80);
  setTimeout(()=>{ if(sec>0 && !tick){ timer=sec; updateClock(); startTimer(); } },260);
}
...
function skipCurrentSet(){
  ...
  if(setNo<totalSets){
    ...
    v676824StartCustomRestAfterSet();
...
function startTimer(){if(tick)return;tick=setInterval(()=>{timer=Math.max(0,timer-1);updateClock();if(timer===0){pauseTimer();if(navigator.vibrate)navigator.vibrate([250,100,250]);
  // V67.68.25 — nos treinos personalizados, ao terminar o descanso o relógio
  // volta ao tempo cheio e fica PARADO. Uma nova contagem só começa quando
  // o militar concluir ou pular a série atual, igual ao fluxo dos treinos prontos.
  if(String(activePlanName||'').startsWith('Personalizado — ')){
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('sw-v67-68-25.js', {updateViaCache:'none'}));
...
localStorage.setItem('t2_app_version','v67.68.25');
...
async function v6743Registration(){
  ...
  if(!reg)reg=await navigator.serviceWorker.register('sw-v67-68-25.js', {updateViaCache:'none'});
