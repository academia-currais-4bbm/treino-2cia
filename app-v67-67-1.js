...
function v67621WorkoutActive(){
  try{return !!localStorage.getItem('t2_active_plan')}catch(e){return !!workoutStartedAt}
}
function v67621GetMusic(){
  if(v67621Music)return v67621Music;
  const a=new Audio('trilha-treino-2cia.mp3?v=67.62.5');
...
/* ===== V67.67.1 — LOCALIZAÇÃO UNIVERSAL: GPS COM RETORNO + LOCAIS RÁPIDOS ===== */
let v67670ActivityLocation={mode:'',label:'',lat:null,lng:null,accuracy:null,status:'',manualOpen:false};
...
function v67670ResetActivityLocation(){v67670ActivityLocation={mode:'',label:'',lat:null,lng:null,accuracy:null,status:'',manualOpen:false};v67670RenderLocationHosts()}
function v67670LocationCard(){
  ...
  const status=v.status || (v.label?(v.mode==='gps'?'✓ GPS capturado e local identificado.':'✓ Local informado.'):(gpsOk?`✓ Localização atual capturada com sucesso${v.accuracy?` (precisão aproximada: ${v.accuracy} m)`:''}.`:'Nenhuma localização adicionada.'));
  ...
  return `<div class="activity-location-card v67670-location-card"><b>📍 Local da atividade <small class="field-optional">opcional</small></b><p>Se ainda estiver no local, use o GPS. Se já tiver saído, escolha a Academia da 2ª CIA ou informe outro local.</p><div class="location-actions"><button type="button" onclick="v67670CaptureLocation()">USAR LOCALIZAÇÃO ATUAL</button><button type="button" onclick="v67670ChooseManualLocation()">INFORMAR LOCAL</button></div>${manual}<div class="activity-location-status">${v67670Esc(status)}</div>${v.label&&!v.manualOther?`<div class="v67671-selected-location">📍 ${v67670Esc(v.label)}</div>`:''}</div>`;
}
function v67670RenderLocationHosts(){['freeActivityLocationHost','planActivityLocationHost','coreActivityLocationHost','cardioActivityLocationHost'].forEach(id=>{const h=byId(id);if(h)h.innerHTML=v67670LocationCard()})}
function v67670LocationInput(value){v67670ActivityLocation.label=String(value||'').trim().slice(0,80);v67670ActivityLocation.mode='manual';v67670ActivityLocation.status=v67670ActivityLocation.label?'✓ Outro local informado.':'Digite o local da atividade.';const active=document.querySelector('.view.active .activity-location-status');if(active)active.textContent=v67670ActivityLocation.status}
function v67670ChooseManualLocation(){v67670ActivityLocation.manualOpen=true;v67670ActivityLocation.status='Escolha um local pré-definido ou selecione Outro local.';v67670RenderLocationHosts()}
function v67671UseDefaultLocation(){v67670ActivityLocation={mode:'manual',label:V67671_DEFAULT_LOCATION,lat:null,lng:null,accuracy:null,status:'✓ Local pré-definido selecionado.',manualOpen:true,manualOther:false};v67670RenderLocationHosts()}
function v67671UseOtherLocation(){v67670ActivityLocation.mode='manual';v67670ActivityLocation.label='';v67670ActivityLocation.lat=null;v67670ActivityLocation.lng=null;v67670ActivityLocation.accuracy=null;v67670ActivityLocation.status='Digite o local da atividade.';v67670ActivityLocation.manualOpen=true;v67670ActivityLocation.manualOther=true;v67670RenderLocationHosts();setTimeout(()=>document.querySelector('.view.active .v67670-location-input')?.focus(),0)}
async function v67670CaptureLocation(){
  ...
  v67670ActivityLocation.status='⏳ Obtendo localização atual…';v67670ActivityLocation.manualOpen=false;v67670RenderLocationHosts();
  try{
    ...
    const c=pos.coords;
    v67670ActivityLocation={mode:'gps',label:'',lat:Number(c.latitude.toFixed(5)),lng:Number(c.longitude.toFixed(5)),accuracy:Math.round(c.accuracy||0),status:`✓ Localização atual capturada com sucesso${c.accuracy?` (precisão aproximada: ${Math.round(c.accuracy)} m)`:''}.`,manualOpen:false,manualOther:false};
...
function v67670LocationFields(){const v=v67670ActivityLocation,label=String(v.label||'').trim();return {activityLocationMode:v.mode||null,activityLocationLabel:label||null,activityLocationGps:v.mode==='gps'?{lat:v.lat,lng:v.lng,accuracy:v.accuracy}:null,feedLocation:label||null}}
...
async function captureCardioLocation(){
  const {input,status}=activeCardioLocationEls();
  if(status)status.textContent='Capturando GPS…';
  try{
    const pos=await getServicePosition(),c=pos.coords;
    cardioActivityLocation={mode:'gps',label:String(input?.value||'').trim(),lat:Number(c.latitude.toFixed(5)),lng:Number(c.longitude.toFixed(5)),accuracy:Math.round(c.accuracy||0)};
    if(status)status.textContent=`✓ GPS capturado${c.accuracy?` • precisão aproximada ${Math.round(c.accuracy)} m`:''}. Dê um nome ao local acima para exibir no Feed.`;
    input?.focus(); saveNavigationState('cardioWorkout');
  }catch(e){if(status)status.textContent='Não foi possível obter o GPS. Você pode informar o local manualmente.';}
}
function chooseCardioManualLocation(){const {input,status}=activeCardioLocationEls();cardioActivityLocation={mode:'manual',label:String(input?.value||'').trim(),lat:null,lng:null,accuracy:null};if(status)status.textContent='Digite o local da atividade. A localização informada será exibida como registro.';input?.focus();}
...
function v67630MetricData(record){
  ...
  }else if(record?.workoutType==='free'){
    out.push(['◆',`${(record.muscleGroups||[]).length}`,'GRUPOS']);out.push(['✓','CONCLUÍDO','STATUS']);
  }else{
    out.push(['◆',`${v67630FmtInt(record.exercisesDone||record.exerciseCount)}`,'EXERCÍCIOS']);out.push(['↻',`${v67630FmtInt(record.sets)}`,'SÉRIES']);out.push(['▥',`${v67630FmtInt(record.volume)} kg`,'VOLUME TOTAL']);
  }
  return out;
}
...
async function v67630BuildShareCard(record){
  ...
  // Nome real sobre uma placa opaca, escondendo o nome demonstrativo da arte-base.
  v67630RoundRect(x,190,1375,700,118,58,'rgba(2,2,2,.97)','#6b0d10');x.textAlign='center';x.fillStyle='#fff';x.font='900 38px Arial';x.fillText(v6748FirstName(),W/2,1425);x.fillStyle='#aaa';x.font='700 20px Arial';x.fillText('TREINO 2ª CIA',W/2,1460);
  return c;
}
async function v67630ShareResult(){
  ...
  const btn=document.getElementById('v67630ShareBtn');const old=btn?.textContent;if(btn){btn.disabled=true;btn.textContent='GERANDO CARD…'}
  try{
    const canvas=await v67630BuildShareCard(record);const blob=await new Promise(res=>canvas.toBlob(res,'image/png',.96));if(!blob)throw new Error('Falha ao gerar imagem');
    const file=new File([blob],`treino-2cia-premium-${Date.now()}.png`,{type:'image/png'});
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){await navigator.share({files:[file],title:'Treino 2ª CIA',text:'Treino concluído. Missão cumprida!'});}
    else{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),1500)}
  }catch(e){if(e?.name!=='AbortError'){console.error('Compartilhar resultado:',e);alert('Não foi possível compartilhar o card agora. Tente novamente.')}}finally{if(btn){btn.disabled=false;btn.textContent=old||'COMPARTILHAR RESULTADO'}}
}
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden'){
    // V67.62.0 — trocar temporariamente para WhatsApp/câmera/etc. NÃO pausa,
    // NÃO muda de tela e NÃO manda o militar para a Home. Apenas persiste o
    // ponto exato do treino para proteção caso o Android encerre o processo.
    v67604SaveActivePlanState();
    saveNavigationState(document.querySelector('.view.active')?.id||'home');
  }
